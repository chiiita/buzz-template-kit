 ...
 *{box-sizing:border-box} body{margin:0;font-family:'Zen Kaku Gothic New',sans-serif;background:#2a2a2e;color:#eee;display:flex;height:100vh;overflow:hidden}
 ...
 #bar{display:flex;flex-wrap:wrap;gap:6px;margin-bottom:12px} #bar button{flex:1;background:#E0352B;color:#fff;border:0;border-radius:8px;padding:11px;font-size:13px;font-weight:700;cursor:pointer} #bar button.sec{background:#555;flex:0 0 auto;padding:11px 12px}
 #topbar{display:flex;justify-content:center;padding:10px;background:#1b1b1e;border-bottom:1px solid #333;flex-shrink:0}
 #movebtn{display:flex;flex-direction:column;align-items:center;gap:2px;min-width:360px;background:#3a3a42;color:#dcdce2;border:2px solid #4a4a52;border-radius:12px;padding:10px 28px;cursor:pointer;font-weight:700;transition:background .12s,border-color .12s,box-shadow .12s}
 #movebtn:hover{border-color:#6a6a74}
 #movebtn.on{background:#1b9e4b;border-color:#34d06b;color:#fff;box-shadow:0 0 0 4px rgba(27,158,75,.28)}
 #movebtn #movestate{font-size:16px;letter-spacing:.5px} #movebtn #movesub{font-size:11px;opacity:.85;font-weight:400}
 ...
 .optsec{background:#26262b;border-radius:8px;padding:10px 11px;margin-bottom:8px}
 .opth{font-size:12px;color:#d8c8b8;font-weight:700;margin-bottom:7px}
 .optlab{font-size:11px;color:#8a9a8a;margin:9px 0 4px}
 .seg{display:flex;gap:5px}
 .seg button{flex:1;background:#33333a;color:#bbb;border:1px solid #44444c;border-radius:6px;padding:7px 4px;font-size:12px;cursor:pointer}
 .seg button.on{background:#E0352B;color:#fff;border-color:#E0352B}
 .optrow{display:flex;align-items:center;gap:8px;margin-top:8px} .optrow input[type=color]{width:46px;height:34px;border:0;background:none;cursor:pointer;flex-shrink:0;padding:0}
 .swrow{display:flex;gap:5px;flex-wrap:wrap;margin-top:6px} .sw{width:26px;height:26px;border-radius:5px;border:1px solid rgba(255,255,255,.22);cursor:pointer;padding:0} .addsw{background:#3a3a42!important;color:#9b9;font-size:15px;line-height:1;display:flex;align-items:center;justify-content:center}
 .optfile{width:100%;color:#ccc;font-size:11px;margin-top:6px} .optrange{width:100%;margin-top:2px}
 .optsel{width:100%;background:#33333a;color:#eee;border:1px solid #44444c;border-radius:6px;padding:7px;font-size:13px}
 .optrow2{display:flex;align-items:center;gap:8px;margin-top:6px} .optrow2 input[type=color]{width:40px;height:30px;border:0;background:none;cursor:pointer;padding:0} .optlab2{font-size:12px;color:#aaa;flex:1}
 .rs{background:#444;color:#ccc;border:0;border-radius:5px;padding:5px 9px;font-size:10px;cursor:pointer}
 .elrow{display:flex;gap:5px;flex-wrap:wrap} .elrow button{flex:1;min-width:46px;background:#33333a;color:#ccc;border:1px solid #44444c;border-radius:6px;padding:7px 4px;font-size:12px;cursor:pointer} .elrow button.on{background:#E0352B;color:#fff;border-color:#E0352B}
 .posgrid{display:grid;grid-template-columns:repeat(3,1fr);gap:4px;width:96px;margin-top:4px} .posgrid button{aspect-ratio:1;background:#33333a;border:1px solid #44444c;border-radius:4px;cursor:pointer;padding:0} .posgrid button.on{background:#E0352B;border-color:#E0352B}
 #bar button:disabled{opacity:.35;cursor:default}
 #movebtn.on{background:#1b9e4b!important;color:#fff}
 .movemode .draggable{cursor:move} .movemode .draggable:hover{outline:2px dashed rgba(224,53,43,.65);outline-offset:2px} .movemode .selblock{outline:2px solid #E0352B!important;outline-offset:2px}
 .freeel:hover{outline:2px dashed rgba(43,108,223,.7);outline-offset:2px} .selfree{outline:2px solid #2D6CDF!important;outline-offset:2px}
 #preview{position:relative} #ovwarn{display:none;position:absolute;top:18px;left:50%;transform:translateX(-50%);background:#B5281F;color:#fff;font-size:12px;padding:6px 14px;border-radius:8px;z-index:5;box-shadow:0 4px 14px rgba(0,0,0,.4)}
 #delbtn{display:none;position:fixed;z-index:40;width:30px;height:30px;border-radius:50%;background:#B5281F;color:#fff;border:2px solid #fff;align-items:center;justify-content:center;cursor:pointer;font-size:15px;line-height:1;box-shadow:0 2px 10px rgba(0,0,0,.45);padding:0}
 #ver{position:absolute;top:6px;right:10px;font-size:10px;color:#666;z-index:6}
...
<div id="mid"><div id="topbar"><button id="movebtn" onclick="toggleMove()"><span id="movestate">🔒 編集モード：オフ</span><span id="movesub">押すと文字・画像を動かせます</span></button></div><div id="preview"><div id="stage"><div id="card"></div></div><div id="ovwarn">⚠ 文字がはみ出している可能性</div><div id="ver">build 2026-06-11c</div></div><div id="deckbar"></div></div>
<div id="form"><div id="bar"><button onclick="doPrint()">デッキをPDF保存</button><button class="sec" id="undobtn" onclick="undo()" title="元に戻す (Ctrl+Z)" disabled>↶</button><button class="sec" id="redobtn" onclick="redo()" title="やり直し (Ctrl+Y)" disabled>↷</button><button class="sec" onclick="togglePresets()">構成</button><button class="sec" onclick="toggleIcons()">アイコン</button><button class="sec" onclick="newDeck()">新規</button><button class="sec" onclick="exportDeck()">書出</button><button class="sec" onclick="document.getElementById('imp').click()">読込</button><input id="imp" type="file" accept="application/json" style="display:none" onchange="importDeck(event)"></div><div id="slideopt"></div><div id="fields"></div>
...
function applyEl(root,s,live){if(!root)return;const ch=root.children,ps=s.pos||{},es=s.el||{},t=eff(s);for(let i=0;i<ch.length;i++){const el=ch[i],p=ps[i],e=es[i]||{};
 el.style.translate=p?(p[0]+'px '+p[1]+'px'):'';
 el.style.scale=e.sc?String(e.sc):'';
...
async function exportPng(all){let toPng;try{const m=await import('https://cdn.jsdelivr.net/npm/html-to-image@1.11.13/+esm');toPng=m.toPng;}catch(e){alert('PNG書き出し用ライブラリを読み込めませんでした。オンライン環境でお試しください。');return;}
 const idxs=all?deck.map(function(_,i){return i;}):[cur];
 for(let n=0;n<idxs.length;n++){const i=idxs[n];const box=document.createElement('div');box.className='pslide';box.style.cssText='position:fixed;left:-99999px;top:0;width:1080px;height:1350px';document.body.appendChild(box);paint(box,deck[i],false);
  try{const url=await toPng(box,{width:1080,height:1350,pixelRatio:1,cacheBust:true});const a=document.createElement('a');a.href=url;a.download='slide_'+String(i+1).padStart(2,'0')+'.png';a.click();}catch(e){alert('スライド'+(i+1)+'のPNG化に失敗: '+e.message);}
  box.remove();await new Promise(function(r){setTimeout(r,250);});}}
...
function hideDelBtn(){const b=document.getElementById('delbtn');if(b)b.style.display='none';}
function positionDelBtn(){const b=document.getElementById('delbtn');if(!b)return;if(!moveMode||(selFree<0&&selBlock<0)){b.style.display='none';return;}const el=document.querySelector(selFree>=0?'#card .selfree':'#card .selblock');if(!el){b.style.display='none';return;}const r=el.getBoundingClientRect();b.style.display='flex';b.style.left=(r.right-12)+'px';b.style.top=(r.top-12)+'px';}
