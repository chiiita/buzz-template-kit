...
<div id="form"><div id="bar"><button onclick="exportPng(false)">🖼 この画像を保存</button><button onclick="exportPng(true)">全画像を保存</button><button onclick="openPick()">選んで保存</button><button class="sec" id="undobtn" onclick="undo()" title="元に戻す (Ctrl+Z)" disabled>↶</button><button class="sec" id="redobtn" onclick="redo()" title="やり直し (Ctrl+Y)" disabled>↷</button><button class="sec" onclick="doPrint()" title="まとめてPDFで保存">PDF</button><button class="sec" onclick="togglePresets()">構成</button><button class="sec" onclick="toggleIcons()">アイコン</button><button class="sec" onclick="resetSlide()">リセット</button><button class="sec" onclick="newDeck()">新規</button><button class="sec" onclick="exportDeck()">書出</button><button class="sec" onclick="document.getElementById('imp').click()">読込</button><input id="imp" type="file" accept="application/json" style="display:none" onchange="importDeck(event)"></div><div id="slideopt"></div><div id="fields"></div>
...
function applyEl(root,s,live){if(!root)return;const ch=root.children,ps=s.pos||{},es=s.el||{},t=eff(s);for(let i=0;i<ch.length;i++){const el=ch[i],p=ps[i],e=es[i]||{};
 ...
 el.style.scale=e.sc?String(e.sc):'';if(e.sc)el.style.transformOrigin='top left';
