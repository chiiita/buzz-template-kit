...
<div id="mid"><div id="preview"><div id="stage"><div id="card"></div></div><div id="ovwarn">⚠ 文字がはみ出している可能性</div></div><div id="deckbar"></div></div>
...
function fitScale(){const sc=Math.min((window.innerWidth-280-340-60)/1080,(window.innerHeight-210)/1350);curScale=Math.max(0.08,sc);document.getElementById('stage').style.transform='scale('+curScale+')';}
...
function enableFreeDrag(root,s){root.querySelectorAll('.freeel').forEach(function(el){const idx=+el.dataset.fi;el.style.cursor='move';el.onmousedown=function(ev){ev.preventDefault();ev.stopPropagation();selFree=idx;selBlock=-1;document.querySelectorAll('#card .selfree').forEach(function(x){x.classList.remove('selfree');});el.classList.add('selfree');buildSlideOpt();const fe=s.free[idx],sx=ev.clientX,sy=ev.clientY,ox=fe.x,oy=fe.y;function mv(e){fe.x=Math.round(ox+(e.clientX-sx)/curScale);fe.y=Math.round(oy+(e.clientY-sy)/curScale);el.style.left=fe.x+'px';el.style.top=fe.y+'px';const sn=computeSnap(cardCoords(el));if(sn.dx!=null){fe.x=Math.round(fe.x+sn.dx);el.style.left=fe.x+'px';}if(sn.dy!=null){fe.y=Math.round(fe.y+sn.dy);el.style.top=fe.y+'px';}showGuide(sn.gx,sn.gy);}function up(){document.removeEventListener('mousemove',mv);document.removeEventListener('mouseup',up);clearGuide();save();renderDeck();}document.addEventListener('mousemove',mv);document.addEventListener('mouseup',up);};});}
...
function renderCard(){const c=document.getElementById('card');const root=paint(c,slide(),moveMode);if(moveMode)enableDrag(root);checkOverflow();}
function enableDrag(root){if(!root)return;root.classList.add('movemode');const ch=root.children;for(let i=0;i<ch.length;i++){(function(el,idx){if(el.classList.contains('freeel')||el.classList.contains('logoel'))return;el.classList.add('draggable');if(idx===selBlock)el.classList.add('selblock');el.onmousedown=function(ev){ev.preventDefault();ev.stopPropagation();selBlock=idx;document.querySelectorAll('#card .selblock').forEach(x=>x.classList.remove('selblock'));el.classList.add('selblock');buildSlideOpt();const s=slide();const base=(s.pos&&s.pos[idx])?s.pos[idx]:[0,0];const sx=ev.clientX,sy=ev.clientY;let tmp=base.slice();function mv(e){const dx=(e.clientX-sx)/curScale,dy=(e.clientY-sy)/curScale;tmp=[Math.round(base[0]+dx),Math.round(base[1]+dy)];el.style.translate=tmp[0]+'px '+tmp[1]+'px';const sn=computeSnap(cardCoords(el));if(sn.dx!=null)tmp[0]=Math.round(tmp[0]+sn.dx);if(sn.dy!=null)tmp[1]=Math.round(tmp[1]+sn.dy);if(sn.dx!=null||sn.dy!=null)el.style.translate=tmp[0]+'px '+tmp[1]+'px';showGuide(sn.gx,sn.gy);}function up(){document.removeEventListener('mousemove',mv);document.removeEventListener('mouseup',up);clearGuide();if(!s.pos)s.pos={};s.pos[idx]=tmp;save();renderDeck();}document.addEventListener('mousemove',mv);document.addEventListener('mouseup',up);};})(ch[i],i);}}
function toggleMove(){moveMode=!moveMode;selBlock=-1;selFree=-1;document.getElementById('movebtn').classList.toggle('on',moveMode);buildSlideOpt();renderCard();}
