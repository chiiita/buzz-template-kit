...
// ===== インフォ図解ヘルパー（NOGUCHI系・YT/note両対応・w,h可変・線画アイコン＋大数字＋黄マーカー＋淡色カード）=====
...
function infoHero(w,h,d){const cy='#1aa3d8';
  return `<div style="${infoCard(w,h,'#eaf6fb')}align-items:center;padding:0 ${Math.round(w*0.07)}px;">`
    ...
    +`</div>`
  +`</div>`;}
function infoStat3(w,h,d){const cy='#1aa3d8';const pad=Math.round(w*0.04);const gap=Math.round(w*0.025);const cw=Math.round((w-pad*2-gap*2)/3);
...
const TEMPLATES = [
  ...
  { id:'okiru7', name:'おきるママ⑦ イベント告知', cat:'おきるママ', fmt:'youtube',
    ...
    render:(d,t)=>{const photo=d.photo?('background:url('+d.photo+') center/cover no-repeat;'):('background:#6a5a4a;');
      ...
      +'</div>';} },
  // ⑧ パネル＋白&黄2行＋×＋2ロゴ＋上赤キッカー（おきるママ/03）
  { id:'okiru8', name:'おきるママ⑧ パネル白黄＋2ロゴ', cat:'おきるママ', fmt:'youtube',
    fields:[
      {key:'photo',label:'背景写真をアップ',type:'file',def:''},
      {key:'top',label:'上 キッカー（【】赤）',def:'この順番【真似するだけ】'},
      {key:'p1',label:'パネル1段目（白）',def:'1日1時間で稼ぐ'},
      {key:'p2',label:'パネル2段目（黄色・改行可）',def:'Threads × note 副業'},
      {key:'logo1',label:'ロゴ画像 左下1(任意)',type:'file',def:''},
      {key:'logo2',label:'ロゴ画像 左下2(任意)',type:'file',def:''},
      {key:'mascot',label:'マスコット 右(任意)',type:'file',def:''}],
    render:(d,t)=>{const mas=d.mascot||BLOB_URI;const photo=d.photo?('background:url('+d.photo+') center/cover no-repeat;'):('background:'+t.panelSoft+';');const tp=String(d.top).replace(/【([^】]*)】/g,'<span style="color:#e02418;">$1</span>');
      return '<div style="width:1280px;height:720px;box-sizing:border-box;position:relative;overflow:hidden;display:flex;flex-direction:column;justify-content:center;align-items:center;padding:30px 60px;font-family:'+t.head+';color:#1a1a1a;'+photo+'">'
        +(d.photo?'<div style="position:absolute;inset:0;background:rgba(255,255,255,.08);"></div>':'')
        +'<div style="position:absolute;top:30px;left:50%;transform:translateX(-50%);display:flex;font-size:56px;font-weight:900;color:#fff;text-shadow:'+outline('#1f1f1f')+';">'+tp+'</div>'
        +'<div style="position:relative;display:flex;flex-direction:column;align-items:center;background:rgba(247,243,233,.5);border-radius:24px;padding:24px 56px;margin-top:30px;">'
          +'<div style="display:flex;font-size:92px;font-weight:900;color:#fff;text-shadow:'+outline('#1f1f1f')+';">'+d.p1+'</div>'
          +'<div style="display:flex;flex-direction:column;align-items:center;font-size:128px;font-weight:900;color:#FFE24A;line-height:1.0;text-shadow:'+outline('#1f1f1f')+';">'+nl(d.p2)+'</div>'
        +'</div>'
        +(d.logo1?'<img src="'+d.logo1+'" style="position:absolute;left:40px;bottom:40px;width:120px;"/>':'')
        +(d.logo2?'<img src="'+d.logo2+'" style="position:absolute;left:172px;bottom:40px;width:120px;"/>':'')
        +'<img src="'+mas+'" style="position:absolute;right:30px;top:130px;width:170px;"/>'
      +'</div>';} },
  // ⑨ 写真直書き＋上小白＋特大黄色＋下＋大ロゴ（おきるママ/04・05）
  { id:'okiru9', name:'おきるママ⑨ 写真直書き＋大ロゴ', cat:'おきるママ', fmt:'youtube',
    fields:[
      {key:'photo',label:'背景写真をアップ',type:'file',def:''},
      {key:'top',label:'上 小（白・【】黄・改行可）',def:'2026年\n在宅で【月5万円！】'},
      {key:'big',label:'下 特大（黄色袋文字・改行可）',def:'note副業の始め方'},
      {key:'logo',label:'大ロゴ画像(任意)',type:'file',def:''},
      {key:'mascot',label:'マスコット 右下(任意)',type:'file',def:''}],
    render:(d,t)=>{const mas=d.mascot||BLOB_URI;const photo=d.photo?('background:url('+d.photo+') center/cover no-repeat;'):('background:#2a2a2a;');const tp=function(s){return String(s).split('\n').filter(function(x){return x.length;}).map(function(line){return '<div style="display:flex;">'+line.replace(/【([^】]*)】/g,'<span style="color:#FFE24A;">$1</span>')+'</div>';}).join('');};
      return '<div style="width:1280px;height:720px;box-sizing:border-box;position:relative;overflow:hidden;display:flex;flex-direction:column;justify-content:space-between;align-items:flex-start;padding:40px 60px;font-family:'+t.head+';color:#fff;'+photo+'">'
        +(d.photo?'<div style="position:absolute;inset:0;background:rgba(0,0,0,.2);"></div>':'')
        +(d.logo?'<img src="'+d.logo+'" style="position:absolute;right:50px;top:60px;width:300px;"/>':'')
        +'<div style="position:relative;display:flex;flex-direction:column;font-size:72px;font-weight:900;line-height:1.15;text-shadow:'+outline('#1f1f1f')+';">'+tp(d.top)+'</div>'
        +'<div style="position:relative;display:flex;flex-direction:column;font-size:150px;font-weight:900;line-height:1.0;color:#FFE24A;text-shadow:'+outline('#1f1f1f')+';">'+nl(d.big)+'</div>'
        +'<img src="'+mas+'" style="position:absolute;right:20px;bottom:10px;width:175px;"/>'
      +'</div>';} },
  // ⑩ 白背景 全面袋文字3段（黒/黄/黒）＋丸ロゴ＋上手書き（おきるママ/06）
  { id:'okiru10', name:'おきるママ⑩ 全面袋文字3段', cat:'おきるママ', fmt:'youtube',
    fields:[
      {key:'photo',label:'背景写真(任意・薄め)',type:'file',def:''},
      {key:'top',label:'左上（黒袋文字）',def:'2026年'},
      {key:'kicker',label:'右上 手書き帯',def:'理解するとバズる！'},
      {key:'l1',label:'1段目（黒袋文字）',def:'Threads'},
      {key:'l2',label:'2段目（黄色袋文字）',def:'アルゴリズム'},
      {key:'l3',label:'3段目（黒袋文字）',def:'徹底解説'},
      {key:'mascot',label:'丸ロゴ マスコット 左(任意)',type:'file',def:''}],
    render:(d,t)=>{const mas=d.mascot||BLOB_URI;const photo=d.photo?('background:url('+d.photo+') center/cover no-repeat;'):('background:#efece7;');
      return '<div style="width:1280px;height:720px;box-sizing:border-box;position:relative;overflow:hidden;display:flex;flex-direction:column;justify-content:center;align-items:center;padding:20px 50px;font-family:'+t.head+';color:#1a1a1a;'+photo+'">'
        +(d.photo?'<div style="position:absolute;inset:0;background:rgba(255,255,255,.45);"></div>':'')
        +'<div style="position:absolute;top:24px;left:0;width:100%;display:flex;align-items:center;justify-content:center;"><div style="display:flex;font-size:70px;font-weight:900;color:#1a1a1a;text-shadow:'+outline('#ffffff')+';margin-right:24px;">'+d.top+'</div><div style="display:flex;font-family:Yomogi;font-size:48px;font-weight:700;color:#1a1a1a;">＼ '+d.kicker+' ／</div></div>'
        +'<div style="position:relative;display:flex;flex-direction:column;align-items:center;margin-top:60px;">'
          +'<div style="display:flex;font-size:130px;font-weight:900;color:#1a1a1a;line-height:1.0;text-shadow:'+outline('#ffffff')+';">'+d.l1+'</div>'
          +'<div style="display:flex;font-size:130px;font-weight:900;color:#FFE24A;line-height:1.0;text-shadow:'+outline('#1f1f1f')+';">'+d.l2+'</div>'
          +'<div style="display:flex;font-size:130px;font-weight:900;color:#1a1a1a;line-height:1.0;text-shadow:'+outline('#ffffff')+';">'+d.l3+'</div>'
        +'</div>'
        +'<div style="position:absolute;left:34px;bottom:40px;display:flex;align-items:center;justify-content:center;width:150px;height:150px;border-radius:50%;background:#fff;border:3px solid #ddd;"><img src="'+mas+'" style="width:96px;height:88px;"/></div>'
      +'</div>';} },
  // ⑪ パネル＋上小白(手書き括弧)＋大見出し赤黄混在＋バッジ/2ロゴ＋マスコット（おきるママ/09・10）
  { id:'okiru11', name:'おきるママ⑪ パネル赤黄＋2ロゴ', cat:'おきるママ', fmt:'youtube',
    fields:[
      {key:'photo',label:'背景写真をアップ',type:'file',def:''},
      {key:'badge',label:'赤バッジ(任意)',def:'2026最新版！'},
      {key:'kicker',label:'パネル上 手書き帯',def:'毎日収入が入ってくる'},
      {key:'big',label:'大見出し（【】赤・《》黄・改行可）',def:'月5万の【不労所得】を\n《得る方法》'},
      {key:'logo1',label:'ロゴ 左上(任意)',type:'file',def:''},
      {key:'logo2',label:'ロゴ 右上(任意)',type:'file',def:''},
      {key:'mascot',label:'マスコット 右(任意)',type:'file',def:''}],
    render:(d,t)=>{const mas=d.mascot||BLOB_URI;const photo=d.photo?('background:url('+d.photo+') center/cover no-repeat;'):('background:'+t.panelSoft+';');const mk=function(s){return String(s).split('\n').filter(function(x){return x.length;}).map(function(line){return '<div style="display:flex;">'+line.replace(/【([^】]*)】/g,'<span style="color:#e02418;">$1</span>').replace(/《([^》]*)》/g,'<span style="color:#FFE24A;">$1</span>')+'</div>';}).join('');};
      return '<div style="width:1280px;height:720px;box-sizing:border-box;position:relative;overflow:hidden;display:flex;flex-direction:column;justify-content:center;align-items:center;padding:30px 60px;font-family:'+t.head+';color:#fff;'+photo+'">'
        +(d.photo?'<div style="position:absolute;inset:0;background:rgba(0,0,0,.12);"></div>':'')
        +(d.logo1?'<img src="'+d.logo1+'" style="position:absolute;left:40px;top:34px;width:120px;"/>':'')
        +(d.logo2?'<img src="'+d.logo2+'" style="position:absolute;right:40px;top:34px;width:120px;"/>':'')
        +(d.badge?'<div style="position:absolute;top:40px;left:50%;transform:translateX(-50%);display:flex;background:#e02418;color:#fff;font-size:44px;font-weight:900;padding:6px 24px;border-radius:10px;text-shadow:'+outline('#7a120c')+';">'+d.badge+'</div>':'')
        +'<div style="position:relative;display:flex;flex-direction:column;align-items:center;background:rgba(0,0,0,.32);border-radius:24px;padding:26px 56px;margin-top:30px;">'
          +'<div style="display:flex;font-family:Yomogi;font-size:42px;font-weight:700;color:#fff;margin-bottom:6px;">＼ '+d.kicker+' ／</div>'
          +'<div style="display:flex;flex-direction:column;align-items:center;font-size:118px;font-weight:900;line-height:1.05;text-shadow:'+outline('#1f1f1f')+';">'+mk(d.big)+'</div>'
        +'</div>'
        +'<img src="'+mas+'" style="position:absolute;right:24px;bottom:10px;width:190px;"/>'
      +'</div>';} },
  // ⑫ ラジオ/音声波形（脱サラジオ系・2マスコット）（おきるママ/25・20系の音声回）
  { id:'okiru12', name:'おきるママ⑫ ラジオ/音声', cat:'おきるママ', fmt:'youtube',
    fields:[
      {key:'label',label:'上 番組名（手書き黄）',def:'おきるラジオ #06'},
      {key:'title',label:'中央 タイトル（白・改行可）',def:'主婦のスレッズ副業の\n始祖がキターーーー！！'},
      {key:'mascotL',label:'左 マスコット(任意)',type:'file',def:''},
      {key:'mascotR',label:'右 マスコット(任意)',type:'file',def:''}],
    render:(d,t)=>{const ml=d.mascotL||BLOB_URI;const mr=d.mascotR||BLOB_URI;
      const wave=[40,70,30,90,55,100,45,80,35,65,50].map(function(h){return '<div style="display:flex;width:10px;height:'+h+'px;background:#FFE24A;border-radius:5px;margin:0 4px;"></div>';}).join('');
      return '<div style="width:1280px;height:720px;box-sizing:border-box;position:relative;overflow:hidden;display:flex;flex-direction:column;justify-content:center;align-items:center;padding:60px;font-family:'+t.head+';color:#fff;background:linear-gradient(135deg,#6a6a6a,#4a4a4a);">'
        +'<div style="position:absolute;top:40px;left:50%;transform:translateX(-50%);display:flex;font-family:Yomogi;font-size:46px;font-weight:700;color:#FFE24A;letter-spacing:3px;">'+d.label+'</div>'
        +'<div style="position:relative;display:flex;flex-direction:column;align-items:center;font-size:78px;font-weight:900;line-height:1.25;text-shadow:0 2px 8px rgba(0,0,0,.5);">'+nl(d.title)+'</div>'
        +'<div style="position:absolute;left:50%;bottom:60px;transform:translateX(-50%);display:flex;align-items:center;justify-content:center;"><div style="display:flex;align-items:center;justify-content:center;width:64px;height:64px;border-radius:50%;background:#FFE24A;margin-right:14px;">'+icon('message-circle','#4a4a4a',36,2.4)+'</div>'+wave+'</div>'
  ...
  // 提供13 数字特大＋人物（勉強配信・青背景）
  { id:'yt_num_live', name:'数字特大＋人物（配信）', cat:'サムネ', fmt:'youtube',
    fields:[
      {key:'photo',label:'背景(任意・無は青)',type:'file',def:''},
      {key:'num',label:'特大数字',def:'10'},
      {key:'unit',label:'単位（縦2字可）',def:'時間'},
      {key:'big',label:'下 大見出し（改行可）',def:'ぶっ通し勉強配信'},
      {key:'ep',label:'右上 話数',def:'#390'},
      {key:'person',label:'人物 右(任意)',type:'file',def:''}],
    render:(d,t)=>{const photo=d.photo?('background:url('+d.photo+') center/cover no-repeat;'):('background:linear-gradient(135deg,#2a5cc8,#102a60);');
      const uc=String(d.unit).split('').map(function(c){return '<div style="display:flex;">'+c+'</div>';}).join('');
      return '<div style="width:1280px;height:720px;box-sizing:border-box;position:relative;overflow:hidden;display:flex;flex-direction:column;justify-content:flex-end;align-items:flex-start;padding:0 60px 24px;font-family:'+t.head+';color:#fff;'+photo+'">'
        +(d.person?'<img src="'+d.person+'" style="position:absolute;right:0;bottom:0;width:470px;height:auto;"/>':'')
        +(d.ep?'<div style="position:absolute;top:24px;right:36px;display:flex;font-size:48px;font-weight:900;text-shadow:'+outline('#102a60')+';">'+d.ep+'</div>':'')
        +'<div style="position:absolute;left:50px;top:10px;display:flex;align-items:flex-start;"><div style="display:flex;font-size:360px;font-weight:900;line-height:.9;text-shadow:'+outline('#102a60')+';">'+d.num+'</div><div style="display:flex;flex-direction:column;font-size:96px;font-weight:900;margin-top:80px;text-shadow:'+outline('#102a60')+';">'+uc+'</div></div>'
        +'<div style="position:relative;display:flex;flex-direction:column;font-size:120px;font-weight:900;line-height:1.0;text-shadow:'+outline('#102a60')+';">'+nl(d.big)+'</div>'
      +'</div>';} },
  // 提供09 6分割人物グリッド＋名前ラベル＋中央タイトル（インタビュー型）
  { id:'yt_grid6', name:'6分割人物＋名前', cat:'サムネ', fmt:'youtube',
    fields:[
      {key:'i1',label:'人物1(任意)',type:'file',def:''},{key:'n1',label:'名前1',def:'田中碧'},
      {key:'i2',label:'人物2(任意)',type:'file',def:''},{key:'n2',label:'名前2',def:'遠藤航'},
      {key:'i3',label:'人物3(任意)',type:'file',def:''},{key:'n3',label:'名前3',def:'久保建英'},
      {key:'i4',label:'人物4(任意)',type:'file',def:''},{key:'n4',label:'名前4',def:'堂安律'},
      {key:'i5',label:'人物5(任意)',type:'file',def:''},{key:'n5',label:'名前5',def:'前田大然'},
      {key:'i6',label:'人物6(任意)',type:'file',def:''},{key:'n6',label:'名前6',def:'鎌田大地'},
      {key:'kicker',label:'中央 上ラベル',def:'日英字幕 -MF・FW編-'},
      {key:'title',label:'中央 タイトル',def:'W杯日本代表の英語力'}],
    render:(d,t)=>{const cell=function(im,nm){return '<div style="display:flex;flex-direction:column;justify-content:flex-end;align-items:flex-start;width:426px;height:360px;padding:8px;background:'+(im?('url('+im+') center/cover no-repeat'):'#555')+';"><div style="display:flex;background:rgba(0,0,0,.62);color:#fff;font-size:30px;font-weight:900;padding:2px 12px;">'+nm+'</div></div>';};
      return '<div style="width:1280px;height:720px;box-sizing:border-box;position:relative;overflow:hidden;display:flex;font-family:'+t.head+';">'
        +'<div style="display:flex;flex-wrap:wrap;width:1280px;height:720px;">'+cell(d.i1,d.n1)+cell(d.i2,d.n2)+cell(d.i3,d.n3)+cell(d.i4,d.n4)+cell(d.i5,d.n5)+cell(d.i6,d.n6)+'</div>'
        +'<div style="position:absolute;top:50%;left:50%;transform:translate(-50%,-50%);display:flex;flex-direction:column;align-items:center;color:#fff;">'
          +'<div style="display:flex;background:#e02418;color:#fff;font-size:34px;font-weight:900;padding:4px 18px;text-shadow:'+outline('#7a120c')+';margin-bottom:6px;">'+d.kicker+'</div>'
          +'<div style="display:flex;font-size:88px;font-weight:900;text-shadow:'+outline('#1f1f1f')+';">'+d.title+'</div>'
        +'</div>'
      +'</div>';} },

  // 提供10 Claude Code 成果報告（白背景＋黒大文字＋数字＋ロボ→対象＋顔右）
  { id:'yt_cc_result', name:'Claude Code 成果報告（白背景）', cat:'サムネ', fmt:'youtube',
    fields:[
      {key:'photo',label:'背景(任意・無は白)',type:'file',def:''},
      {key:'line1',label:'上 見出し（青下線・改行可）',def:'iOSアプリ開発で'},
      {key:'big',label:'特大（黒・改行可）',def:'+10万円/日'},
      {key:'label',label:'ロボ下ラベル',def:'Claude Code'},
      {key:'target',label:'対象アイコン画像 中央下(任意)',type:'file',def:''},
      {key:'person',label:'顔写真 右(任意)',type:'file',def:''}],
    render:(d,t)=>{const photo=d.photo?('background:url('+d.photo+') center/cover no-repeat;'):('background:#ece8e1;');
      return '<div style="width:1280px;height:720px;box-sizing:border-box;position:relative;overflow:hidden;display:flex;flex-direction:column;justify-content:flex-start;align-items:flex-start;padding:60px 70px;font-family:'+t.head+';color:#1a1a1a;'+photo+'">'
        +(d.person?'<img src="'+d.person+'" style="position:absolute;right:0;bottom:0;width:440px;height:auto;"/>':'')
        +'<div style="position:relative;display:flex;flex-direction:column;font-size:80px;font-weight:900;line-height:1.1;">'+nl(d.line1)+'</div>'
        +'<div style="position:relative;display:flex;width:480px;height:6px;background:#1aa0c0;margin:6px 0 4px;border-radius:3px;"></div>'
        +'<div style="position:relative;display:flex;flex-direction:column;font-size:170px;font-weight:900;line-height:1.0;">'+nl(d.big)+'</div>'
  ...
  { id:'yt_ai_explain', name:'Y6 AI解説＋数字（人物右）', cat:'アーカイブ', fmt:'youtube',
    fields:[
  ...
  { id:'yt_expert_box', name:'Y7 専門家ボックス煽り', cat:'アーカイブ', fmt:'youtube',
    fields:[
  ...
  { id:'yt_contrast', name:'Y8 顔ドアップ左右対比', cat:'アーカイブ', fmt:'youtube',
    fields:[
  ...
  { id:'yt_seminar', name:'Y9 セミナー告知（にぎやか）', cat:'アーカイブ', fmt:'youtube',
    fields:[
  ...
  { id:'yt_webinar', name:'Y10 ウェビナー告知（清潔）', cat:'アーカイブ', fmt:'youtube',
    fields:[
      {key:'logo',label:'ロゴ画像 左上(任意)',type:'file',def:''},
  ...
  { id:'yt_seminar_v', name:'Y11 セミナー告知（人物左）', cat:'アーカイブ', fmt:'youtube',
    fields:[
  ...
  { id:'yt_sns_proof', name:'Y12 SNS講座＋実績3箱', cat:'アーカイブ', fmt:'youtube',
    fields:[
      {key:'photo',label:'背景写真(任意)',type:'file',def:''},
  ...
  { id:'yt_talk2', name:'Y13 2人対談告知', cat:'アーカイブ', fmt:'youtube',
    fields:[
  ...
  { id:'yt_recruit', name:'Y14 採用・英字コーポレート', cat:'アーカイブ', fmt:'youtube',
    fields:[
  ...
  { id:'yt_vlog_food', name:'Y15 Vlog料理・暮らし', cat:'アーカイブ', fmt:'youtube',
    fields:[
      {key:'photo',label:'背景写真をアップ',type:'file',def:''},
  ...
  { id:'yt_vlog_travel', name:'Y16 トラベルVlog', cat:'アーカイブ', fmt:'youtube',
    fields:[
      {key:'photo',label:'背景写真をアップ',type:'file',def:''},
  ...
  // Y17 ← #5 やわらかコピー（写真＋左寄せ詩的コピー＋ロゴ枠） pin/789115165944035971
  { id:'yt_soft_copy', name:'Y17 やわらかコピー', cat:'アーカイブ', fmt:'youtube',
    fields:[
      {key:'photo',label:'背景写真をアップ',type:'file',def:''},
      {key:'copy',label:'コピー（改行で多行）',def:'なんでもない\nふだんを、\n宝物にしよう。'},
      {key:'logo1',label:'ロゴ枠 上',def:'ふだん'},
      {key:'logo2',label:'ロゴ枠 下',def:'プレミアム'}],
    render:(d,t)=>{const photo=d.photo?('background:url('+d.photo+') center/cover no-repeat;'):('background:#7d7468;');
      return '<div style="width:1280px;height:720px;box-sizing:border-box;position:relative;overflow:hidden;display:flex;flex-direction:column;justify-content:flex-start;align-items:flex-start;padding:110px 90px;font-family:'+t.head+';'+photo+'">'
        +'<div style="position:relative;display:flex;flex-direction:column;font-size:76px;font-weight:700;line-height:1.5;color:#fff;text-shadow:0 2px 12px rgba(0,0,0,.55);">'+nl(d.copy)+'</div>'
        +'<div style="position:absolute;left:90px;bottom:60px;display:flex;flex-direction:column;align-items:center;border:4px solid #fff;border-radius:6px;padding:10px 24px;color:#fff;text-shadow:0 1px 6px rgba(0,0,0,.5);"><div style="display:flex;font-size:28px;font-weight:700;">'+d.logo1+'</div><div style="display:flex;font-size:42px;font-weight:900;">'+d.logo2+'</div></div>'
      +'</div>';} },
  // Y18 ← #6 コラージュVlog（4分割写真＋中央楕円パネル＋ハッシュタグ） pin/1057149712546811915
  { id:'yt_collage', name:'Y18 コラージュVlog', cat:'アーカイブ', fmt:'youtube',
    fields:[
      {key:'img1',label:'写真 左上(任意)',type:'file',def:''},
      {key:'img2',label:'写真 右上(任意)',type:'file',def:''},
      {key:'img3',label:'写真 左下(任意)',type:'file',def:''},
      {key:'img4',label:'写真 右下(任意)',type:'file',def:''},
      {key:'tags',label:'左上ハッシュタグ（改行）',def:'#ヘアセット\n#コーデ\n#宝物な思い出'},
      {key:'kicker',label:'楕円 上',def:'大好きなみんなと過ごす'},
      {key:'title',label:'楕円 中央（改行可）',def:'お誕生日会の1日'},
      {key:'date',label:'楕円 下',def:'Birthday Event 2023'}],
    render:(d,t)=>{const cell=function(im){return '<div style="display:flex;width:640px;height:360px;background:'+(im?('url('+im+') center/cover no-repeat'):'#cfc6ba')+';"></div>';};
      const tg=String(d.tags).split('\n').filter(function(x){return x.length;}).map(function(x){return '<div style="display:flex;align-self:flex-start;background:rgba(255,255,255,.86);color:#5a4e42;font-size:24px;font-weight:700;padding:4px 14px;border-radius:6px;margin-bottom:8px;">'+x+'</div>';}).join('');
      return '<div style="width:1280px;height:720px;box-sizing:border-box;position:relative;overflow:hidden;display:flex;font-family:'+t.head+';">'
        +'<div style="display:flex;flex-wrap:wrap;width:1280px;height:720px;">'+cell(d.img1)+cell(d.img2)+cell(d.img3)+cell(d.img4)+'</div>'
        +'<div style="position:absolute;top:30px;left:34px;display:flex;flex-direction:column;">'+tg+'</div>'
        +'<div style="position:absolute;top:50%;left:50%;transform:translate(-50%,-50%);display:flex;flex-direction:column;align-items:center;justify-content:center;width:620px;height:330px;border-radius:50%;background:rgba(255,253,250,.95);color:#5a4e42;padding:30px;">'
          +'<div style="display:flex;font-size:30px;font-weight:700;">＼ '+d.kicker+' ／</div>'
          +'<div style="display:flex;flex-direction:column;align-items:center;font-size:72px;font-weight:900;line-height:1.1;margin:8px 0;">'+nl(d.title)+'</div>'
          +'<div style="display:flex;font-size:28px;font-style:italic;font-weight:700;color:#9c8e7a;">'+d.date+'</div>'
        +'</div>'
      +'</div>';} },
  // Y19 ← #11 休日Vlogポラロイド（白セリフ＋ポラロイド2枚＋手書き矢印） pin/419749627791977872
  { id:'yt_vlog_holiday', name:'Y19 休日Vlogポラロイド', cat:'アーカイブ', fmt:'youtube',
    fields:[
      {key:'photo',label:'背景写真をアップ',type:'file',def:''},
      {key:'title',label:'英字大タイトル',def:'A DAY IN MY LIFE'},
      {key:'tag',label:'タグ',def:'30代OL休日Vlog'},
      {key:'note',label:'手書き風メモ（改行可）',def:'見つけたオシャレな\nお店もご紹介'},
      {key:'pola1',label:'ポラロイド1(任意)',type:'file',def:''},
      {key:'cap1',label:'ポラ1 説明',def:'ベーカリー'},
      {key:'pola2',label:'ポラロイド2(任意)',type:'file',def:''},
      {key:'cap2',label:'ポラ2 説明',def:'カフェ'}],
    render:(d,t)=>{const photo=d.photo?('background:url('+d.photo+') center/cover no-repeat;'):('background:#8a8377;');
      const pola=function(im,cap,rot,off){return '<div style="position:absolute;right:'+off+'px;bottom:38px;display:flex;flex-direction:column;align-items:center;background:#fff;padding:12px 12px 8px;transform:rotate('+rot+'deg);box-shadow:0 8px 22px rgba(0,0,0,.4);"><div style="display:flex;width:240px;height:170px;background:'+(im?('url('+im+') center/cover no-repeat'):'#bcb6ab')+';"></div><div style="display:flex;font-size:22px;font-weight:700;color:#444;margin-top:6px;">'+cap+'</div></div>';};
      return '<div style="width:1280px;height:720px;box-sizing:border-box;position:relative;overflow:hidden;display:flex;flex-direction:column;align-items:flex-start;padding:46px 60px;font-family:'+t.head+';'+photo+'">'
        +(d.photo?'<div style="position:absolute;inset:0;background:rgba(0,0,0,.14);"></div>':'')
        +'<div style="position:relative;display:flex;font-size:84px;font-weight:700;letter-spacing:5px;color:#fff;text-shadow:0 2px 12px rgba(0,0,0,.5);">'+d.title+'</div>'
        +'<div style="position:relative;display:flex;margin-top:8px;border:3px solid #fff;color:#fff;font-size:30px;font-weight:900;padding:5px 16px;border-radius:6px;text-shadow:0 1px 6px rgba(0,0,0,.5);">'+d.tag+'</div>'
        +'<div style="position:absolute;left:70px;top:300px;display:flex;flex-direction:column;font-family:Yomogi;font-size:40px;font-weight:700;color:#fff;text-shadow:0 2px 8px rgba(0,0,0,.6);">'+nl(d.note)+'</div>'
        +'<img src="'+ARROW_URI+'" style="position:absolute;left:470px;top:368px;width:110px;"/>'
        +pola(d.pola2,d.cap2,4,60)
        +pola(d.pola1,d.cap1,-5,340)
      +'</div>';} },

  // Y20 ← ガジェット 商品レビュー◯選（商品3枚＋袋文字＋星バッジ） yt_ref3 #4 pin/1117103882599366790
  { id:'yt_review3', name:'Y20 商品レビュー◯選', cat:'アーカイブ', fmt:'youtube',
    fields:[
      {key:'p1',label:'商品写真1(任意)',type:'file',def:''},
      {key:'p2',label:'商品写真2(任意)',type:'file',def:''},
      {key:'p3',label:'商品写真3(任意)',type:'file',def:''},
      {key:'sub',label:'上 小コピー',def:'手元撮影にも最適！'},
      {key:'big',label:'商品名（白袋文字）',def:'ウェブカメラ'},
      {key:'big2',label:'下段（黄色袋文字）',def:'おすすめ3選！'},
      {key:'badge',label:'右下バッジ（改行可）',def:'ベスト\nグッズ図鑑'}],
    render:(d,t)=>{const prod=function(im){return '<div style="display:flex;width:300px;height:230px;background:'+(im?('url('+im+') center/contain no-repeat'):'#243150')+';border-radius:8px;margin:0 12px;"></div>';};
      return '<div style="width:1280px;height:720px;box-sizing:border-box;position:relative;overflow:hidden;display:flex;flex-direction:column;justify-content:flex-end;align-items:flex-start;padding:0 60px 50px;font-family:'+t.head+';color:#fff;background:linear-gradient(160deg,#1b2745,#0e1730);">'
        +'<div style="position:absolute;top:30px;left:0;width:100%;display:flex;justify-content:center;">'+prod(d.p1)+prod(d.p2)+prod(d.p3)+'</div>'
        +'<div style="position:relative;display:flex;font-size:46px;font-weight:900;color:#fff;margin-bottom:6px;">'+d.sub+'</div>'
        +'<div style="position:relative;display:flex;font-size:120px;font-weight:900;color:#fff;line-height:1.0;text-shadow:'+outline('#E0352B')+';">'+d.big+'</div>'
        +'<div style="position:relative;display:flex;font-size:130px;font-weight:900;color:#FFE24A;line-height:1.0;text-shadow:'+outline('#1f1f1f')+';">'+d.big2+'</div>'
        +(d.badge?'<div style="position:absolute;right:50px;bottom:50px;display:flex;flex-direction:column;align-items:center;justify-content:center;width:200px;height:200px;border-radius:50%;background:#16335c;border:5px solid #FFE24A;color:#fff;font-size:34px;font-weight:900;line-height:1.1;text-align:center;"><div style="display:flex;color:#FFE24A;font-size:30px;">★★★</div>'+nl(d.badge)+'</div>':'')
      +'</div>';} },
  // Y21 ← 炎VS氷 二項対決（人物2＋炎氷＋赤青袋文字＋VS） yt_ref3 #5 pin/747386500664040758
  { id:'yt_vs_fire', name:'Y21 炎VS氷 対決', cat:'アーカイブ', fmt:'youtube',
    fields:[
      {key:'top',label:'上 特大（白袋文字・改行可）',def:'ビジネスで大切なのは'},
      {key:'leftImg',label:'左 人物(任意)',type:'file',def:''},
      {key:'rightImg',label:'右 人物(任意)',type:'file',def:''},
      {key:'left',label:'左ワード（改行可）',def:'成功体験'},
      {key:'right',label:'右ワード（改行可）',def:'失敗体験'}],
    render:(d,t)=>{
      return '<div style="width:1280px;height:720px;box-sizing:border-box;position:relative;overflow:hidden;display:flex;font-family:'+t.head+';background:linear-gradient(90deg,#7a3410,#20140c 38%,#14181c 50%,#122a3f 62%,#0e3a5e);">'
        +(d.leftImg?'<img src="'+d.leftImg+'" style="position:absolute;left:0;bottom:0;width:430px;height:auto;"/>':'')
        +(d.rightImg?'<img src="'+d.rightImg+'" style="position:absolute;right:0;bottom:0;width:430px;height:auto;"/>':'')
        +'<div style="position:absolute;top:24px;left:0;width:100%;display:flex;flex-direction:column;align-items:center;font-size:130px;font-weight:900;line-height:1.0;color:#fff;text-shadow:'+outline('#1f1f1f')+';">'+nl(d.top)+'</div>'
        +'<div style="position:absolute;left:300px;top:300px;transform:translateX(-50%);display:flex;flex-direction:column;align-items:center;font-size:110px;font-weight:900;color:#fff;line-height:1.0;text-shadow:'+outline('#c0211a')+';">'+nl(d.left)+'</div>'
        +'<div style="position:absolute;right:300px;top:300px;transform:translateX(50%);display:flex;flex-direction:column;align-items:center;font-size:110px;font-weight:900;color:#fff;line-height:1.0;text-shadow:'+outline('#15457a')+';">'+nl(d.right)+'</div>'
        +'<div style="position:absolute;left:50%;top:420px;transform:translate(-50%,-50%);display:flex;align-items:center;justify-content:center;width:150px;height:150px;border-radius:50%;background:#FFE24A;border:7px solid #1f1f1f;font-size:66px;font-weight:900;font-style:italic;color:#1f1f1f;">VS</div>'
      +'</div>';} },
  // Y22 ← 対談 緑青2分割（2人＋中央 対×談＋黄名＋囲み小見出し） yt_ref3 #7 pin/777926535655741927
  { id:'yt_taidan', name:'Y22 対談（緑青2分割）', cat:'アーカイブ', fmt:'youtube',
    fields:[
      {key:'leftImg',label:'左 人物(任意)',type:'file',def:''},
      {key:'rightImg',label:'右 人物(任意)',type:'file',def:''},
      {key:'leftName',label:'左 名前(英字・改行可)',def:'RYOTARO\nSANO'},
      {key:'rightName',label:'右 名前(英字・改行可)',def:'YUUTA\nNISHITANI'},
      {key:'leftSub',label:'左 小ラベル',def:'時代に求められるマインド'},
      {key:'rightSub',label:'右 小ラベル',def:'未来を切り拓く条件'},
      {key:'label',label:'中央 ラベル',def:'taidan channel'}],
    render:(d,t)=>{const li=d.leftImg?('background:url('+d.leftImg+') center/cover;'):'background:#5fc46a;';const ri=d.rightImg?('background:url('+d.rightImg+') center/cover;'):'background:#39c6f0;';
      return '<div style="width:1280px;height:720px;box-sizing:border-box;position:relative;overflow:hidden;display:flex;font-family:'+t.head+';">'
        +'<div style="display:flex;width:640px;height:720px;'+li+'"></div>'
        +'<div style="display:flex;width:640px;height:720px;'+ri+'"></div>'
        +'<div style="position:absolute;left:50%;top:0;height:720px;transform:translateX(-50%);display:flex;flex-direction:column;align-items:center;justify-content:center;color:#1a1a1a;"><div style="display:flex;font-size:180px;font-weight:900;line-height:.92;">対</div><div style="display:flex;font-size:90px;font-weight:900;color:#FFE24A;line-height:.7;">×</div><div style="display:flex;font-size:30px;font-style:italic;color:#fff;margin:2px 0;">'+d.label+'</div><div style="display:flex;font-size:180px;font-weight:900;line-height:.92;">談</div></div>'
        +'<div style="position:absolute;left:34px;top:30px;display:flex;flex-direction:column;font-size:48px;font-weight:900;color:#fff;line-height:1.0;">'+nl(d.leftName)+'</div>'
        +'<div style="position:absolute;right:34px;bottom:30px;display:flex;flex-direction:column;align-items:flex-end;font-size:48px;font-weight:900;color:#FFE24A;line-height:1.0;">'+nl(d.rightName)+'</div>'
        +'<div style="position:absolute;left:34px;bottom:40px;display:flex;background:#1a1a1a;color:#fff;font-size:24px;font-weight:700;padding:6px 14px;">'+d.leftSub+'</div>'
        +'<div style="position:absolute;right:34px;top:40px;display:flex;background:#1a1a1a;color:#fff;font-size:24px;font-weight:700;padding:6px 14px;">'+d.rightSub+'</div>'
      +'</div>';} },

  // Y23 ← 教育HowTo（緑帯＋電球＋青背景＋紺袋文字＋人物右） yt_ref4 #7 pin/756041856200519597
  { id:'yt_howto', name:'Y23 教育HowTo（電球＋人物右）', cat:'アーカイブ', fmt:'youtube',
    fields:[
      {key:'top',label:'上 緑帯テキスト',def:'超ラクに作れる！'},
      {key:'small',label:'小ラベル',def:'YouTube'},
      {key:'big',label:'特大（紺・白フチ・改行可）',def:'サムネイル'},
      {key:'foot',label:'下段',def:'簡単テンプレート'},
      {key:'person',label:'人物 右(任意)',type:'file',def:''}],
    render:(d,t)=>{const NV='#16335c',GR='#3bb3a0';
      return '<div style="width:1280px;height:720px;box-sizing:border-box;position:relative;overflow:hidden;display:flex;flex-direction:column;justify-content:center;align-items:center;padding:130px 70px 50px;font-family:'+t.head+';color:'+NV+';background:linear-gradient(180deg,#dcf1ed,#bbe5df);">'
        +(d.person?'<img src="'+d.person+'" style="position:absolute;right:0;bottom:0;width:400px;height:auto;"/>':'')
        +'<div style="position:absolute;top:0;left:0;width:100%;display:flex;align-items:center;justify-content:center;background:'+GR+';padding:12px 0;"><div style="display:flex;align-items:center;justify-content:center;width:60px;height:60px;border-radius:50%;background:#fff;margin-right:16px;">'+icon('lightbulb','#F2B900',38,2)+'</div><div style="display:flex;font-size:48px;font-weight:900;color:#1a1a1a;">'+d.top+'</div></div>'
        +'<div style="position:relative;display:flex;font-size:40px;font-weight:700;letter-spacing:10px;color:#5a6b7a;margin-bottom:6px;">'+d.small+'</div>'
        +'<div style="position:relative;display:flex;flex-direction:column;align-items:center;font-size:150px;font-weight:900;line-height:1.0;color:'+NV+';text-shadow:'+outline('#ffffff')+';">'+nl(d.big)+'</div>'
        +'<div style="position:relative;display:flex;margin-top:14px;background:'+NV+';color:#fff;font-size:48px;font-weight:900;padding:8px 28px;border-radius:10px;">'+d.foot+'</div>'
      +'</div>';} },
  // Y24 ← 王道バズ・数字（金キラ背景＋上帯＋赤帯＋特大数字＋人物右） yt_ref4 #11 pin/591660469835320177
  { id:'yt_money_buzz', name:'Y24 王道バズ・数字（金キラ）', cat:'アーカイブ', fmt:'youtube',
    fields:[
      {key:'top',label:'上 黒帯テキスト',def:'初心者が確実に'},
      {key:'line2',label:'赤帯 白文字',def:'WEBデザインで'},
      {key:'unitL',label:'数字 前',def:'月'},
      {key:'num',label:'特大数字',def:'5'},
      {key:'unitR',label:'数字 後',def:'万'},
      {key:'verb',label:'動詞（紺）',def:'稼ぐ'},
      {key:'foot',label:'小（方法）',def:'方法'},
      {key:'person',label:'人物 右(任意)',type:'file',def:''}],
    render:(d,t)=>{const RD='#E0352B',NV='#16335c';
      return '<div style="width:1280px;height:720px;box-sizing:border-box;position:relative;overflow:hidden;display:flex;flex-direction:column;justify-content:flex-end;align-items:flex-start;padding:24px 70px 26px;font-family:'+t.head+';background:radial-gradient(circle at 50% 44%, #ffe98a, #f3c01e);">'
        +(d.person?'<img src="'+d.person+'" style="position:absolute;right:0;bottom:0;width:420px;height:auto;"/>':'')
        +'<div style="position:absolute;top:26px;left:60px;display:flex;background:#1a1a1a;color:#fff;font-size:40px;font-weight:900;padding:8px 22px;border-radius:10px;">'+d.top+'</div>'
        +'<div style="position:relative;display:flex;background:'+RD+';color:#fff;font-size:64px;font-weight:900;padding:6px 24px;border-radius:10px;text-shadow:'+outline('#8e1f18')+';margin-bottom:10px;">'+d.line2+'</div>'
        +'<div style="position:relative;display:flex;align-items:baseline;">'
          +'<div style="display:flex;font-size:96px;font-weight:900;color:'+RD+';text-shadow:'+outline('#ffffff')+';">'+d.unitL+'</div>'
          +'<div style="display:flex;font-size:240px;font-weight:900;color:'+RD+';line-height:.9;text-shadow:'+outline('#ffffff')+';">'+d.num+'</div>'
          +'<div style="display:flex;font-size:110px;font-weight:900;color:'+RD+';text-shadow:'+outline('#ffffff')+';">'+d.unitR+'</div>'
          +'<div style="display:flex;font-size:150px;font-weight:900;color:'+NV+';text-shadow:'+outline('#ffffff')+';margin-left:8px;">'+d.verb+'</div>'
          +'<div style="display:flex;font-size:50px;font-weight:900;color:#1a1a1a;margin-left:6px;">'+d.foot+'</div>'
        +'</div>'
      +'</div>';} },

  // ===== 王道バズ汎用型（収集バズ群 yt_ref* の共通パターンを反映・ゴシック既定） =====
  { id:'yt_nsen', name:'Y25 ◯選・番号', cat:'アーカイブ', fmt:'youtube',
    fields:[{key:'photo',label:'背景写真(任意)',type:'file',def:''},{key:'kicker',label:'上 黒帯',def:'保存しないと損'},{key:'topic',label:'テーマ（紺・改行可）',def:'神アプリ'},{key:'count',label:'数',def:'7'},{key:'unit',label:'単位',def:'選'},{key:'person',label:'人物 右(任意)',type:'file',def:''}],
    render:(d,t)=>{const NV='#16335c';const photo=d.photo?('background:url('+d.photo+') center/cover no-repeat;'):('background:radial-gradient(circle at 45% 45%, #ffe98a, #f3c01e);');
      return '<div style="width:1280px;height:720px;box-sizing:border-box;position:relative;overflow:hidden;display:flex;flex-direction:row;justify-content:flex-start;align-items:center;padding:60px 70px;font-family:'+t.head+';color:'+NV+';'+photo+'">'
        +(d.person?'<img src="'+d.person+'" style="position:absolute;right:0;bottom:0;width:400px;height:auto;"/>':'')
        +'<div style="position:absolute;top:26px;left:60px;display:flex;background:#1a1a1a;color:#fff;font-size:38px;font-weight:900;padding:8px 22px;border-radius:10px;">'+d.kicker+'</div>'
        +'<div style="position:relative;display:flex;flex-direction:column;font-size:128px;font-weight:900;line-height:1.05;text-shadow:'+outline('#ffffff')+';margin-right:24px;">'+nl(d.topic)+'</div>'
        +'<div style="position:relative;display:flex;align-items:center;justify-content:center;width:280px;height:280px;border-radius:50%;background:#E0352B;border:7px solid #fff;"><div style="display:flex;align-items:baseline;color:#fff;text-shadow:'+outline('#8e1f18')+';"><div style="display:flex;font-size:190px;font-weight:900;line-height:1;">'+d.count+'</div><div style="display:flex;font-size:70px;font-weight:900;">'+d.unit+'</div></div></div>'
      +'</div>';} },
  { id:'yt_rank1', name:'Y26 ランキング1位', cat:'アーカイブ', fmt:'youtube',
    fields:[{key:'photo',label:'背景写真(任意)',type:'file',def:''},{key:'kicker',label:'上 黒帯',def:'プロが本気で選んだ'},{key:'rank',label:'順位の数字',def:'1'},{key:'title',label:'タイトル（紺・改行可）',def:'稼げる副業\nランキング'},{key:'person',label:'人物 右(任意)',type:'file',def:''}],
    render:(d,t)=>{const NV='#16335c';const photo=d.photo?('background:url('+d.photo+') center/cover no-repeat;'):('background:radial-gradient(circle at 45% 45%, #fff0c0, #e9c64e);');
      return '<div style="width:1280px;height:720px;box-sizing:border-box;position:relative;overflow:hidden;display:flex;flex-direction:row;justify-content:flex-start;align-items:center;padding:60px 70px;font-family:'+t.head+';color:'+NV+';'+photo+'">'
        +(d.person?'<img src="'+d.person+'" style="position:absolute;right:0;bottom:0;width:380px;height:auto;"/>':'')
        +'<div style="position:absolute;top:26px;left:60px;display:flex;background:#1a1a1a;color:#fff;font-size:38px;font-weight:900;padding:8px 22px;border-radius:10px;">'+d.kicker+'</div>'
        +'<div style="position:relative;display:flex;align-items:baseline;margin-right:24px;"><div style="display:flex;font-size:70px;font-weight:900;color:#C9A227;">第</div><div style="display:flex;font-size:300px;font-weight:900;line-height:.9;color:#FFD23C;text-shadow:'+outline('#9c7a14')+';">'+d.rank+'</div><div style="display:flex;font-size:70px;font-weight:900;color:#C9A227;">位</div></div>'
        +'<div style="position:relative;display:flex;flex-direction:column;font-size:100px;font-weight:900;line-height:1.18;text-shadow:'+outline('#ffffff')+';">'+nl(d.title)+'</div>'
      +'</div>';} },
  { id:'yt_ba', name:'Y27 Before→After', cat:'アーカイブ', fmt:'youtube',
    fields:[{key:'title',label:'上 タイトル（改行可）',def:'人生変わった'},{key:'bl',label:'Before 文',def:'毎月赤字'},{key:'al',label:'After 文',def:'月20万貯金'},{key:'bimg',label:'Before写真(任意)',type:'file',def:''},{key:'aimg',label:'After写真(任意)',type:'file',def:''}],
    render:(d,t)=>{const bi=d.bimg?('background:url('+d.bimg+') center/cover;'):'background:#6b6b6b;';const ai=d.aimg?('background:url('+d.aimg+') center/cover;'):('background:'+t.accent+';');
      return '<div style="width:1280px;height:720px;box-sizing:border-box;position:relative;overflow:hidden;display:flex;font-family:'+t.head+';">'
        ...
        +'<div style="position:absolute;top:30px;left:50%;transform:translateX(-50%);display:flex;flex-direction:column;align-items:center;font-size:60px;font-weight:900;color:#fff;text-shadow:'+outline('#1f1f1f')+';">'+nl(d.title)+'</div>'
      ...
      +'</div>';} },
  { id:'yt_q', name:'Y28 疑問フック', cat:'アーカイブ', fmt:'youtube',
    fields:[{key:'photo',label:'背景写真(任意)',type:'file',def:''},{key:'kicker',label:'上 黒帯',def:'9割が間違えてる'},{key:'q',label:'問い（紺・改行可）',def:'なぜお金が\n残らない？'},{key:'hint',label:'引き',def:'答えはたった1つ'},{key:'person',label:'人物 右(任意)',type:'file',def:''}],
    render:(d,t)=>{const NV='#16335c';const photo=d.photo?('background:url('+d.photo+') center/cover no-repeat;'):('background:linear-gradient(135deg,#eef5fc,#d7e8f8);');
      return '<div style="width:1280px;height:720px;box-sizing:border-box;position:relative;overflow:hidden;display:flex;flex-direction:column;justify-content:center;align-items:flex-start;padding:90px 70px;font-family:'+t.head+';color:'+NV+';'+photo+'">'
        +(d.person?'<img src="'+d.person+'" style="position:absolute;right:0;bottom:0;width:380px;height:auto;"/>':'')
        +'<div style="position:absolute;top:26px;left:60px;display:flex;background:#1a1a1a;color:#fff;font-size:38px;font-weight:900;padding:8px 22px;border-radius:10px;">'+d.kicker+'</div>'
        +'<div style="position:absolute;right:60px;top:60px;display:flex;font-size:360px;font-weight:900;line-height:.85;color:#FFD23C;text-shadow:'+outline('#1f1f1f')+';">?</div>'
        +'<div style="position:relative;display:flex;flex-direction:column;font-size:108px;font-weight:900;line-height:1.16;text-shadow:'+outline('#ffffff')+';">'+nl(d.q)+'</div>'
      ...
      +'</div>';} },
  { id:'yt_warn', name:'Y29 損失回避・黒地警告', cat:'アーカイブ', fmt:'youtube',
    fields:[{key:'badge',label:'赤バッジ',def:'知らないと損'},{key:'big',label:'特大（白・改行可）',def:'その節約\n逆効果'},{key:'foot',label:'下段（黄）',def:'今すぐ見直して'},{key:'person',label:'人物 右(任意)',type:'file',def:''}],
    render:(d,t)=>{
      return '<div style="width:1280px;height:720px;box-sizing:border-box;position:relative;overflow:hidden;display:flex;flex-direction:column;justify-content:center;align-items:flex-start;padding:80px 70px;font-family:'+t.head+';color:#fff;background:radial-gradient(circle at 40% 40%, #2a2a30, #0c0c10);">'
        +(d.person?'<img src="'+d.person+'" style="position:absolute;right:0;bottom:0;width:400px;height:auto;"/>':'')
        +'<div style="position:relative;display:flex;background:#E0352B;color:#fff;font-size:54px;font-weight:900;padding:6px 26px;border-radius:10px;margin-bottom:16px;text-shadow:'+outline('#8e1f18')+';">'+d.badge+'</div>'
        +'<div style="position:relative;display:flex;flex-direction:column;font-size:150px;font-weight:900;line-height:1.02;color:#fff;text-shadow:'+outline('#1f1f1f')+';">'+nl(d.big)+'</div>'
        +'<div style="position:relative;display:flex;margin-top:16px;background:#FFE24A;color:#1f1f1f;font-size:46px;font-weight:900;padding:8px 24px;border-radius:10px;">'+d.foot+'</div>'
      +'</div>';} },
  { id:'yt_expose', name:'Y30 暴露・真実', cat:'アーカイブ', fmt:'youtube',
    fields:[{key:'pre',label:'前フリ（白・改行可）',def:'誰も言わない'},{key:'big',label:'対象（黄・改行可）',def:'副業の'},{key:'word',label:'真実ワード（赤）',def:'闇'},{key:'person',label:'人物 右(任意)',type:'file',def:''}],
    render:(d,t)=>{
      return '<div style="width:1280px;height:720px;box-sizing:border-box;position:relative;overflow:hidden;display:flex;flex-direction:column;justify-content:center;align-items:flex-start;padding:80px 70px;font-family:'+t.head+';color:#fff;background:radial-gradient(circle at 60% 45%, #5a1410, #120808);">'
        +(d.person?'<img src="'+d.person+'" style="position:absolute;right:0;bottom:0;width:400px;height:auto;"/>':'')
        +'<div style="position:relative;display:flex;flex-direction:column;font-size:64px;font-weight:900;line-height:1.1;text-shadow:'+outline('#1f1f1f')+';">'+nl(d.pre)+'</div>'
        +'<div style="position:relative;display:flex;align-items:baseline;margin-top:6px;"><div style="display:flex;font-size:130px;font-weight:900;color:#FFE24A;text-shadow:'+outline('#1f1f1f')+';">'+d.big+'</div><div style="display:flex;font-size:210px;font-weight:900;color:#E0352B;line-height:.9;text-shadow:'+outline('#ffffff')+';margin-left:6px;">'+d.word+'</div></div>'
      +'</div>';} },

  // Y31 ← 美容 商品訴求（モデル右＋斜めリボン＋大コピー＋ブランド） yt_ref5 #3 pin（明朝推奨）
  { id:'yt_beauty_product', name:'Y31 美容・商品訴求', cat:'アーカイブ', fmt:'youtube',
    fields:[
      {key:'model',label:'モデル写真 右(任意)',type:'file',def:''},
      {key:'product',label:'商品写真 中央(任意)',type:'file',def:''},
      {key:'brand',label:'ブランド名 左上',def:'Pure Natural Premium'},
      {key:'ribbon',label:'斜めリボン',def:'美白ケアの新習慣！'},
      {key:'sub',label:'サブ',def:'「美白美容液」+「角質ケア」がこれ1本'},
      {key:'big1',label:'大コピー1',def:'本気の'},
      {key:'big2',label:'大コピー2（アクセント）',def:'シミ対策！'}],
    render:(d,t)=>{const NV='#1f3a6b',BL='#2f7fe0';
      return '<div style="width:1280px;height:720px;box-sizing:border-box;position:relative;overflow:hidden;display:flex;flex-direction:column;justify-content:flex-end;align-items:flex-start;padding:60px 70px;font-family:'+t.head+';color:'+NV+';background:linear-gradient(135deg,#e2effb,#bcd8f4);">'
        +(d.model?'<img src="'+d.model+'" style="position:absolute;right:0;bottom:0;width:420px;height:auto;"/>':'')
        +(d.product?'<img src="'+d.product+'" style="position:absolute;right:380px;top:60px;width:200px;height:auto;"/>':'')
        +'<div style="position:absolute;top:54px;left:70px;display:flex;font-size:36px;font-weight:700;letter-spacing:2px;color:'+NV+';">'+d.brand+'</div>'
        +(d.ribbon?'<div style="position:absolute;top:120px;left:430px;display:flex;background:'+NV+';color:#fff;font-size:34px;font-weight:900;padding:8px 22px;transform:rotate(-6deg);">＼ '+d.ribbon+' ／</div>':'')
        +'<div style="position:relative;display:flex;font-size:34px;font-weight:700;color:'+NV+';margin-bottom:8px;">'+d.sub+'</div>'
        +'<div style="position:relative;display:flex;align-items:baseline;font-weight:900;line-height:1.0;"><div style="display:flex;font-size:120px;color:'+NV+';">'+d.big1+'</div><div style="display:flex;font-size:140px;color:'+BL+';">'+d.big2+'</div></div>'
      +'</div>';} },
  // Y32 ← 美容 上品コスメ（商品中央＋左明朝コピー＋英字＋受賞バッジ） yt_ref5 #6 pin（明朝推奨）
  { id:'yt_beauty_lux', name:'Y32 美容・上品コスメ', cat:'アーカイブ', fmt:'youtube',
    fields:[
      {key:'product',label:'商品写真 右(任意)',type:'file',def:''},
      {key:'copy1',label:'コピー上（小）',def:'とろり生搾りオイルで'},
      {key:'copy2',label:'コピー大（改行可）',def:'うるりキラ髪'},
      {key:'eng',label:'英字',def:'SUPER SHINY'},
      {key:'date',label:'発売',def:'3.1 DEBUT'},
      {key:'badge',label:'右上バッジ（改行可）',def:'ベスコス\n65冠'}],
    render:(d,t)=>{const CH='#3a3550';
      return '<div style="width:1280px;height:720px;box-sizing:border-box;position:relative;overflow:hidden;display:flex;flex-direction:column;justify-content:center;align-items:flex-start;padding:60px 80px;font-family:'+t.head+';color:'+CH+';background:linear-gradient(135deg,#efe9f6,#ddd2ec);">'
        +(d.product?'<img src="'+d.product+'" style="position:absolute;right:40px;bottom:0;width:480px;height:auto;"/>':'')
        +(d.badge?'<div style="position:absolute;top:40px;right:46px;display:flex;flex-direction:column;align-items:center;justify-content:center;width:160px;height:160px;border-radius:50%;background:#fff;border:4px solid #b9a44e;color:#7a6a2e;font-size:34px;font-weight:900;line-height:1.05;text-align:center;">'+nl(d.badge)+'</div>':'')
        +'<div style="position:relative;display:flex;font-size:36px;font-weight:700;margin-bottom:6px;">'+d.copy1+'</div>'
        +'<div style="position:relative;display:flex;flex-direction:column;font-size:104px;font-weight:900;line-height:1.1;">'+nl(d.copy2)+'</div>'
        +'<div style="position:relative;display:flex;margin-top:22px;font-size:48px;font-weight:700;letter-spacing:6px;color:#6a5fa0;">'+d.eng+'</div>'
        +'<div style="position:relative;display:flex;margin-top:6px;font-size:30px;font-weight:700;letter-spacing:3px;color:'+CH+';">'+d.date+'</div>'
      +'</div>';} },

  // Y33 ← 料理コラージュ 中央パネル（周囲6料理＋中央白パネル＋赤手書き＋赤帯） yt_ref6 #7 pin/657807089317436283
  { id:'yt_food_collage', name:'Y33 料理コラージュ・中央パネル', cat:'アーカイブ', fmt:'youtube',
    fields:[
      {key:'i1',label:'料理1(任意)',type:'file',def:''},{key:'i2',label:'料理2(任意)',type:'file',def:''},{key:'i3',label:'料理3(任意)',type:'file',def:''},
      {key:'i4',label:'料理4(任意)',type:'file',def:''},{key:'i5',label:'料理5(任意)',type:'file',def:''},{key:'i6',label:'料理6(任意)',type:'file',def:''},
      {key:'kicker',label:'手書きキッカー（赤）',def:'おうちで'},
      {key:'title',label:'タイトル（改行可）',def:'シェフのレシピ'},
      {key:'sub',label:'赤帯サブ',def:'さぬきうどん'}],
    render:(d,t)=>{const cell=function(im){return '<div style="display:flex;width:426px;height:360px;background:'+(im?('url('+im+') center/cover no-repeat'):'#cbb9a3')+';"></div>';};
      return '<div style="width:1280px;height:720px;box-sizing:border-box;position:relative;overflow:hidden;display:flex;font-family:'+t.head+';">'
        +'<div style="display:flex;flex-wrap:wrap;width:1280px;height:720px;">'+cell(d.i1)+cell(d.i2)+cell(d.i3)+cell(d.i4)+cell(d.i5)+cell(d.i6)+'</div>'
        +'<div style="position:absolute;top:50%;left:50%;transform:translate(-50%,-50%);display:flex;flex-direction:column;align-items:center;justify-content:center;width:580px;height:360px;border-radius:30px;background:rgba(255,255,255,.96);color:#2a2a2a;padding:24px;box-shadow:0 10px 30px rgba(0,0,0,.25);">'
          +'<div style="display:flex;font-family:Yomogi;font-size:44px;font-weight:700;color:#E0352B;">・'+d.kicker+'・</div>'
          +'<div style="display:flex;flex-direction:column;align-items:center;font-size:80px;font-weight:900;line-height:1.1;margin:6px 0 14px;">'+nl(d.title)+'</div>'
          +'<div style="display:flex;background:#E0352B;color:#fff;font-size:34px;font-weight:900;padding:8px 30px;border-radius:6px;">'+d.sub+'</div>'
        +'</div>'
      +'</div>';} },
  // Y34 ← 物産展 4分割（料理4枚＋上帯＋左下特大＋日程） yt_ref6 #12 pin/834010424768500289
  { id:'yt_food_event', name:'Y34 物産展・4分割', cat:'アーカイブ', fmt:'youtube',
    fields:[
      {key:'i1',label:'料理1(任意)',type:'file',def:''},{key:'i2',label:'料理2(任意)',type:'file',def:''},{key:'i3',label:'料理3(任意)',type:'file',def:''},{key:'i4',label:'料理4(任意)',type:'file',def:''},
      {key:'topline',label:'上 帯コピー',def:'旬のグルメが大集合！会場で美味巡りを'},
      {key:'pre',label:'特大の前',def:'初夏の'},
      {key:'big',label:'特大（緑）',def:'大北海道展'},
      {key:'date',label:'日程',def:'4.26→5.9 ／ 7階 催会場'}],
    render:(d,t)=>{const GN='#3a9e3a';const C=['#e87a2a','#2f8fd0','#4caa4c','#e06a8a'];const cell=function(im,i){return '<div style="display:flex;width:320px;height:720px;background:'+(im?('url('+im+') center/cover no-repeat'):C[i])+';"></div>';};
      return '<div style="width:1280px;height:720px;box-sizing:border-box;position:relative;overflow:hidden;display:flex;font-family:'+t.head+';">'
        +'<div style="display:flex;width:1280px;height:720px;">'+cell(d.i1,0)+cell(d.i2,1)+cell(d.i3,2)+cell(d.i4,3)+'</div>'
        +'<div style="position:absolute;top:0;left:0;width:100%;display:flex;align-items:center;justify-content:center;background:rgba(0,0,0,.55);color:#fff;font-size:34px;font-weight:900;padding:10px 0;">'+d.topline+'</div>'
  ...
  { id:'yt_game_challenge', name:'Y35 ゲーム実況・特大煽り', cat:'アーカイブ', fmt:'youtube',
    fields:[
  ...
  { id:'yt_stream_event', name:'Y36 配信・イベント告知', cat:'アーカイブ', fmt:'youtube',
    fields:[
  ...
  // ===== 定番アーキタイプ拡充（ニュース/図解/吹出/比較/TOP5/旅行/セール） =====
  { id:'yt_news', name:'Y37 ニュース速報風', cat:'アーカイブ', fmt:'youtube',
    fields:[{key:'photo',label:'背景(任意)',type:'file',def:''},{key:'badge',label:'赤バッジ',def:'速報'},{key:'title',label:'特大（白・改行可）',def:'まさかの\n新事実'},{key:'ticker',label:'下テロップ',def:'知らないと損する最新情報'},{key:'person',label:'人物 右(任意)',type:'file',def:''}],
    render:(d,t)=>{const photo=d.photo?('background:url('+d.photo+') center/cover no-repeat;'):('background:linear-gradient(135deg,#1c2740,#0c1018);');
      return '<div style="width:1280px;height:720px;box-sizing:border-box;position:relative;overflow:hidden;display:flex;flex-direction:column;justify-content:center;align-items:flex-start;padding:90px 70px 130px;font-family:'+t.head+';color:#fff;'+photo+'">'
        +(d.photo?'<div style="position:absolute;inset:0;background:rgba(0,0,0,.34);"></div>':'')
        +(d.person?'<img src="'+d.person+'" style="position:absolute;right:0;bottom:96px;width:400px;height:auto;"/>':'')
        +'<div style="position:absolute;top:28px;left:60px;display:flex;background:#E0352B;color:#fff;font-size:46px;font-weight:900;padding:8px 26px;border-radius:8px;letter-spacing:4px;">'+d.badge+'</div>'
        +'<div style="position:relative;display:flex;flex-direction:column;font-size:150px;font-weight:900;line-height:1.04;text-shadow:'+outline('#1f1f1f')+';">'+nl(d.title)+'</div>'
        +'<div style="position:absolute;left:0;bottom:0;width:100%;display:flex;align-items:center;background:#E0352B;color:#fff;font-size:40px;font-weight:900;padding:14px 40px;">'+d.ticker+'</div>'
      +'</div>';} },
  { id:'yt_flow3', name:'Y38 図解3ステップ', cat:'アーカイブ', fmt:'youtube',
    fields:[{key:'kicker',label:'上 黒帯',def:'たった3ステップ'},{key:'title',label:'タイトル（紺・改行可）',def:'初心者でもできる'},{key:'a',label:'ステップ1',def:'登録'},{key:'b',label:'ステップ2',def:'設定'},{key:'c',label:'ステップ3',def:'完了'}],
    render:(d,t)=>{const NV='#16335c';const box=function(s){return '<div style="display:flex;align-items:center;justify-content:center;width:280px;height:170px;background:#fff;border:5px solid '+NV+';border-radius:18px;font-size:54px;font-weight:900;color:'+NV+';">'+s+'</div>';};const ar='<div style="display:flex;align-items:center;font-size:80px;font-weight:900;color:'+t.accent+';margin:0 10px;">→</div>';
      return '<div style="width:1280px;height:720px;box-sizing:border-box;position:relative;overflow:hidden;display:flex;flex-direction:column;justify-content:center;align-items:center;padding:70px;font-family:'+t.head+';color:'+NV+';background:linear-gradient(135deg,#eef5fc,#d7e8f8);">'
        +'<div style="position:absolute;top:30px;left:50%;transform:translateX(-50%);display:flex;background:#1a1a1a;color:#fff;font-size:40px;font-weight:900;padding:8px 24px;border-radius:10px;">'+d.kicker+'</div>'
        +'<div style="display:flex;flex-direction:column;align-items:center;font-size:78px;font-weight:900;line-height:1.1;margin-bottom:34px;text-shadow:'+outline('#ffffff')+';">'+nl(d.title)+'</div>'
        +'<div style="display:flex;align-items:center;">'+box(d.a)+ar+box(d.b)+ar+box(d.c)+'</div>'
      +'</div>';} },
  { id:'yt_react', name:'Y39 顔アップ＋吹き出し', cat:'アーカイブ', fmt:'youtube',
    fields:[{key:'photo',label:'背景(任意)',type:'file',def:''},{key:'person',label:'人物 左(任意)',type:'file',def:''},{key:'bubble',label:'吹き出し（改行可）',def:'え、それ\n損してる！'},{key:'big',label:'特大（黄・改行可）',def:'正しいやり方'}],
    render:(d,t)=>{const photo=d.photo?('background:url('+d.photo+') center/cover no-repeat;'):('background:linear-gradient(135deg,#3a3550,#16131f);');
      return '<div style="width:1280px;height:720px;box-sizing:border-box;position:relative;overflow:hidden;display:flex;flex-direction:column;justify-content:center;align-items:flex-end;padding:60px 70px;font-family:'+t.head+';color:#fff;'+photo+'">'
        +(d.person?'<img src="'+d.person+'" style="position:absolute;left:20px;bottom:0;width:440px;height:auto;"/>':'')
        +'<div style="position:absolute;left:430px;top:60px;display:flex;flex-direction:column;background:#fff;border:5px solid #1a1a1a;border-radius:26px;padding:18px 28px;font-size:56px;font-weight:900;color:#1a1a1a;line-height:1.2;">'+nl(d.bubble)+'</div>'
        +'<div style="position:relative;display:flex;flex-direction:column;align-items:flex-end;font-size:150px;font-weight:900;line-height:1.0;color:#FFE24A;text-shadow:'+outline('#1f1f1f')+';">'+nl(d.big)+'</div>'
      +'</div>';} },
  { id:'yt_compare', name:'Y40 ◯×比較表', cat:'アーカイブ', fmt:'youtube',
    fields:[{key:'title',label:'上 タイトル',def:'どっちを選ぶ？'},{key:'an',label:'左 名前',def:'A社'},{key:'bn',label:'右 名前',def:'B社'},{key:'items',label:'項目（改行・左◯右×）',def:'料金が安い\nサポート充実\n初心者向け'}],
    render:(d,t)=>{const NV='#16335c';const rows=String(d.items).split('\n').filter(function(x){return x.length;}).map(function(it){return '<div style="display:flex;align-items:center;border-top:2px solid #d7e0ec;padding:8px 0;"><div style="display:flex;flex:1;font-size:40px;font-weight:900;color:'+NV+';">'+it+'</div><div style="display:flex;width:120px;justify-content:center;">'+icon('check','#2faa55',56,3)+'</div><div style="display:flex;width:120px;justify-content:center;">'+icon('x','#E0352B',52,3)+'</div></div>';}).join('');
      return '<div style="width:1280px;height:720px;box-sizing:border-box;position:relative;overflow:hidden;display:flex;flex-direction:column;justify-content:center;padding:50px 80px;font-family:'+t.head+';color:'+NV+';background:#fff;">'
        +'<div style="display:flex;align-self:center;background:#1a1a1a;color:#fff;font-size:44px;font-weight:900;padding:8px 28px;border-radius:10px;margin-bottom:18px;">'+d.title+'</div>'
        +'<div style="display:flex;align-items:center;padding:6px 0;"><div style="display:flex;flex:1;"></div><div style="display:flex;width:120px;justify-content:center;font-size:36px;font-weight:900;color:#2faa55;">'+d.an+'</div><div style="display:flex;width:120px;justify-content:center;font-size:36px;font-weight:900;color:#E0352B;">'+d.bn+'</div></div>'
        +rows
      +'</div>';} },
  { id:'yt_top5', name:'Y41 TOP5 リスト', cat:'アーカイブ', fmt:'youtube',
    fields:[{key:'photo',label:'背景(任意)',type:'file',def:''},{key:'title',label:'タイトル（改行可）',def:'おすすめ\nアプリTOP5'},{key:'items',label:'5項目（改行）',def:'家計簿アプリ\nポイ活アプリ\nフリマアプリ\n投資アプリ\n節約アプリ'},{key:'person',label:'人物 右(任意)',type:'file',def:''}],
    render:(d,t)=>{const NV='#16335c';const photo=d.photo?('background:url('+d.photo+') center/cover no-repeat;'):('background:radial-gradient(circle at 40% 45%, #ffe98a, #f3c01e);');
      const li=String(d.items).split('\n').filter(function(x){return x.length;}).slice(0,5).map(function(it,i){return '<div style="display:flex;align-items:center;margin-bottom:8px;"><div style="display:flex;align-items:center;justify-content:center;width:54px;height:54px;border-radius:50%;background:#E0352B;color:#fff;font-size:32px;font-weight:900;margin-right:14px;">'+(i+1)+'</div><div style="display:flex;background:#fff;color:'+NV+';font-size:38px;font-weight:900;padding:4px 18px;border-radius:8px;">'+it+'</div></div>';}).join('');
      return '<div style="width:1280px;height:720px;box-sizing:border-box;position:relative;overflow:hidden;display:flex;flex-direction:row;align-items:center;padding:50px 70px;font-family:'+t.head+';color:'+NV+';'+photo+'">'
        +(d.person?'<img src="'+d.person+'" style="position:absolute;right:0;bottom:0;width:360px;height:auto;"/>':'')
        +'<div style="position:relative;display:flex;flex-direction:column;font-size:84px;font-weight:900;line-height:1.1;margin-right:40px;text-shadow:'+outline('#ffffff')+';">'+nl(d.title)+'</div>'
        +'<div style="position:relative;display:flex;flex-direction:column;">'+li+'</div>'
      +'</div>';} },
  { id:'yt_travel_scenic', name:'Y42 旅行・絶景', cat:'アーカイブ', fmt:'youtube',
    fields:[{key:'photo',label:'絶景写真をアップ',type:'file',def:''},{key:'tag',label:'上タグ',def:'国内旅行'},{key:'place',label:'地名 大（改行可）',def:'絶景の旅'},{key:'sub',label:'サブ',def:'死ぬまでに行きたい'}],
    render:(d,t)=>{const photo=d.photo?('background:url('+d.photo+') center/cover no-repeat;'):('background:#5b7a86;');
      return '<div style="width:1280px;height:720px;box-sizing:border-box;position:relative;overflow:hidden;display:flex;flex-direction:column;justify-content:center;align-items:center;padding:60px;font-family:'+t.head+';color:#fff;'+photo+'">'
        +(d.photo?'<div style="position:absolute;inset:0;background:rgba(0,0,0,.22);"></div>':'')
        +'<div style="position:relative;display:flex;border:3px solid #fff;font-size:34px;font-weight:900;letter-spacing:4px;padding:5px 22px;margin-bottom:14px;text-shadow:0 2px 8px rgba(0,0,0,.5);">'+d.tag+'</div>'
        +'<div style="position:relative;display:flex;flex-direction:column;align-items:center;font-size:140px;font-weight:900;line-height:1.05;text-shadow:0 3px 16px rgba(0,0,0,.55);">'+nl(d.place)+'</div>'
        +'<div style="position:relative;display:flex;margin-top:12px;font-size:44px;font-weight:700;text-shadow:0 2px 10px rgba(0,0,0,.6);">'+d.sub+'</div>'
      +'</div>';} },
  { id:'yt_sale', name:'Y43 セール・お得', cat:'アーカイブ', fmt:'youtube',
    fields:[{key:'badge',label:'上バッジ',def:'期間限定'},{key:'pre',label:'特大の前',def:'今だけ'},{key:'num',label:'割引数字',def:'50'},{key:'unit',label:'単位',def:'%OFF'},{key:'foot',label:'下段',def:'見逃すと損！'},{key:'person',label:'人物 右(任意)',type:'file',def:''}],
    render:(d,t)=>{
      return '<div style="width:1280px;height:720px;box-sizing:border-box;position:relative;overflow:hidden;display:flex;flex-direction:column;justify-content:center;align-items:flex-start;padding:70px;font-family:'+t.head+';color:#fff;background:radial-gradient(circle at 45% 45%, #ff5a4d, #c01f16);">'
        +(d.person?'<img src="'+d.person+'" style="position:absolute;right:0;bottom:0;width:400px;height:auto;"/>':'')
        +'<div style="position:absolute;top:30px;left:70px;display:flex;background:#FFE24A;color:#c01f16;font-size:44px;font-weight:900;padding:6px 24px;border-radius:10px;">'+d.badge+'</div>'
        +'<div style="position:relative;display:flex;font-size:64px;font-weight:900;text-shadow:'+outline('#7a140d')+';">'+d.pre+'</div>'
        +'<div style="position:relative;display:flex;align-items:baseline;"><div style="display:flex;font-size:300px;font-weight:900;color:#FFE24A;line-height:.85;text-shadow:'+outline('#7a140d')+';">'+d.num+'</div><div style="display:flex;font-size:120px;font-weight:900;color:#FFE24A;text-shadow:'+outline('#7a140d')+';">'+d.unit+'</div></div>'
        +'<div style="position:relative;display:flex;margin-top:8px;background:#1a1a1a;color:#fff;font-size:44px;font-weight:900;padding:6px 24px;border-radius:8px;">'+d.foot+'</div>'
      +'</div>';} },

  // ===== 定番アーキタイプ拡充2（総集編/ルーティン/キッズ/ダイエット/地図/引用/黒板）→50到達 =====
  { id:'yt_complete', name:'Y44 総集編・完全版', cat:'アーカイブ', fmt:'youtube',
    fields:[{key:'photo',label:'背景(任意)',type:'file',def:''},{key:'badge',label:'金バッジ',def:'完全版'},{key:'title',label:'特大（白・改行可）',def:'これ1本で\n全部わかる'},{key:'sub',label:'下段（金）',def:'保存して何度も見て'},{key:'person',label:'人物 右(任意)',type:'file',def:''}],
    render:(d,t)=>{const photo=d.photo?('background:url('+d.photo+') center/cover no-repeat;'):('background:radial-gradient(circle at 45% 42%, #243150, #0c1326);');
      return '<div style="width:1280px;height:720px;box-sizing:border-box;position:relative;overflow:hidden;display:flex;flex-direction:column;justify-content:center;align-items:flex-start;padding:80px 70px;font-family:'+t.head+';color:#fff;'+photo+'">'
        +(d.photo?'<div style="position:absolute;inset:0;background:rgba(0,0,0,.4);"></div>':'')
        +(d.person?'<img src="'+d.person+'" style="position:absolute;right:0;bottom:0;width:400px;height:auto;"/>':'')
        +'<div style="position:relative;display:flex;align-self:flex-start;background:#e9c64e;color:#3a2a08;font-size:46px;font-weight:900;padding:6px 28px;border-radius:10px;margin-bottom:16px;">'+d.badge+'</div>'
        +'<div style="position:relative;display:flex;flex-direction:column;font-size:140px;font-weight:900;line-height:1.04;text-shadow:'+outline('#1f1f1f')+';">'+nl(d.title)+'</div>'
        +'<div style="position:relative;display:flex;margin-top:14px;font-size:46px;font-weight:900;color:#FFD23C;text-shadow:'+outline('#1f1f1f')+';">'+d.sub+'</div>'
      +'</div>';} },
  { id:'yt_routine', name:'Y45 1日ルーティン', cat:'アーカイブ', fmt:'youtube',
    fields:[{key:'photo',label:'背景写真(任意)',type:'file',def:''},{key:'title',label:'タイトル（改行可）',def:'私の朝\nルーティン'},{key:'items',label:'時刻リスト（改行 "06:00 起床"）',def:'06:00 起床\n06:30 ヨガ\n07:00 朝食'}],
    render:(d,t)=>{const NV='#16335c';const photo=d.photo?('background:url('+d.photo+') center/cover no-repeat;'):('background:linear-gradient(135deg,#fbf3e8,#f3e2c8);');
      const li=String(d.items).split('\n').filter(function(x){return x.length;}).map(function(it){const sp=it.indexOf(' ');const tm=sp>0?it.slice(0,sp):it;const tx=sp>0?it.slice(sp+1):'';return '<div style="display:flex;align-items:center;margin-bottom:10px;"><div style="display:flex;background:'+t.accent+';color:#fff;font-size:34px;font-weight:900;padding:4px 16px;border-radius:8px;margin-right:14px;">'+tm+'</div><div style="display:flex;font-size:40px;font-weight:900;color:'+NV+';">'+tx+'</div></div>';}).join('');
      return '<div style="width:1280px;height:720px;box-sizing:border-box;position:relative;overflow:hidden;display:flex;flex-direction:row;align-items:center;padding:60px 70px;font-family:'+t.head+';color:'+NV+';'+photo+'">'
        +(d.photo?'<div style="position:absolute;inset:0;background:rgba(255,255,255,.12);"></div>':'')
        +'<div style="position:relative;display:flex;flex-direction:column;font-size:96px;font-weight:900;line-height:1.1;margin-right:50px;text-shadow:'+outline('#ffffff')+';">'+nl(d.title)+'</div>'
        +'<div style="position:relative;display:flex;flex-direction:column;">'+li+'</div>'
      +'</div>';} },
  { id:'yt_kids', name:'Y46 キッズ・知育ポップ', cat:'アーカイブ', fmt:'youtube',
    fields:[{key:'title',label:'特大（改行可）',def:'たのしく\nまなぼう！'},{key:'sub',label:'サブ',def:'はじめてのひらがな'},{key:'char',label:'キャラ/人物 右(任意)',type:'file',def:''}],
    render:(d,t)=>{
      return '<div style="width:1280px;height:720px;box-sizing:border-box;position:relative;overflow:hidden;display:flex;flex-direction:column;justify-content:center;align-items:flex-start;padding:70px;font-family:'+t.head+';color:#fff;background:linear-gradient(135deg,#ffd24a,#ff8a3c);">'
        +(d.char?'<img src="'+d.char+'" style="position:absolute;right:0;bottom:0;width:430px;height:auto;"/>':'')
        +'<div style="position:relative;display:flex;flex-direction:column;font-size:150px;font-weight:900;line-height:1.05;color:#fff;text-shadow:'+outline('#e0561f')+';">'+nl(d.title)+'</div>'
        +'<div style="position:relative;display:flex;margin-top:16px;background:#fff;color:#e0561f;font-size:50px;font-weight:900;padding:8px 28px;border-radius:999px;">'+d.sub+'</div>'
      +'</div>';} },
  { id:'yt_diet_ba', name:'Y47 ダイエットBA・数値', cat:'アーカイブ', fmt:'youtube',
    fields:[{key:'title',label:'上 タイトル（改行可）',def:'3ヶ月で激変'},{key:'bimg',label:'Before写真(任意)',type:'file',def:''},{key:'aimg',label:'After写真(任意)',type:'file',def:''},{key:'bw',label:'Before数値',def:'75kg'},{key:'aw',label:'After数値',def:'58kg'}],
    render:(d,t)=>{const bi=d.bimg?('background:url('+d.bimg+') center/cover;'):'background:#6b6b6b;';const ai=d.aimg?('background:url('+d.aimg+') center/cover;'):('background:'+t.accent+';');
      return '<div style="width:1280px;height:720px;box-sizing:border-box;position:relative;overflow:hidden;display:flex;font-family:'+t.head+';">'
        +'<div style="display:flex;flex-direction:column;justify-content:flex-end;align-items:center;width:640px;height:720px;padding:30px;'+bi+'"><div style="display:flex;background:#333;color:#fff;font-size:36px;font-weight:900;padding:6px 20px;border-radius:8px;margin-bottom:10px;">Before</div><div style="display:flex;font-size:110px;font-weight:900;color:#fff;text-shadow:'+outline('#1f1f1f')+';">'+d.bw+'</div></div>'
        +'<div style="display:flex;flex-direction:column;justify-content:flex-end;align-items:center;width:640px;height:720px;padding:30px;'+ai+'"><div style="display:flex;background:#fff;color:'+t.accentDeep+';font-size:36px;font-weight:900;padding:6px 20px;border-radius:8px;margin-bottom:10px;">After</div><div style="display:flex;font-size:120px;font-weight:900;color:#fff;text-shadow:'+outline('#1f1f1f')+';">'+d.aw+'</div></div>'
        +'<div style="position:absolute;top:30px;left:50%;transform:translateX(-50%);display:flex;flex-direction:column;align-items:center;font-size:64px;font-weight:900;color:#fff;text-shadow:'+outline('#1f1f1f')+';">'+nl(d.title)+'</div>'
        +'<div style="position:absolute;top:50%;left:50%;transform:translate(-50%,-50%);display:flex;align-items:center;justify-content:center;width:130px;height:130px;border-radius:50%;background:#FFE24A;border:6px solid #1f1f1f;font-size:54px;font-weight:900;color:#1f1f1f;">→</div>'
      +'</div>';} },
  { id:'yt_map_spot', name:'Y48 地図・スポット紹介', cat:'アーカイブ', fmt:'youtube',
    fields:[{key:'photo',label:'背景写真(任意)',type:'file',def:''},{key:'tag',label:'上タグ',def:'保存版'},{key:'place',label:'地名 大（改行可）',def:'東京\nおすすめカフェ'},{key:'sub',label:'サブ',def:'地元民が通う名店'}],
    render:(d,t)=>{const photo=d.photo?('background:url('+d.photo+') center/cover no-repeat;'):('background:#5a7a6a;');
      return '<div style="width:1280px;height:720px;box-sizing:border-box;position:relative;overflow:hidden;display:flex;flex-direction:column;justify-content:center;align-items:flex-start;padding:70px;font-family:'+t.head+';color:#fff;'+photo+'">'
        +(d.photo?'<div style="position:absolute;inset:0;background:rgba(0,0,0,.3);"></div>':'')
        +'<div style="position:relative;display:flex;align-items:center;margin-bottom:14px;"><div style="display:flex;margin-right:10px;">'+icon('map-pin','#FFE24A',56,2.4)+'</div><div style="display:flex;border:3px solid #fff;font-size:34px;font-weight:900;padding:4px 18px;">'+d.tag+'</div></div>'
        +'<div style="position:relative;display:flex;flex-direction:column;font-size:128px;font-weight:900;line-height:1.05;text-shadow:0 3px 14px rgba(0,0,0,.6);">'+nl(d.place)+'</div>'
        +'<div style="position:relative;display:flex;margin-top:12px;background:'+t.accent+';color:#fff;font-size:42px;font-weight:900;padding:6px 22px;border-radius:8px;">'+d.sub+'</div>'
      +'</div>';} },
  { id:'yt_quote', name:'Y49 引用・証言', cat:'アーカイブ', fmt:'youtube',
    fields:[{key:'photo',label:'背景(任意)',type:'file',def:''},{key:'quote',label:'引用文（改行可）',def:'人生は\n選択の連続だ'},{key:'name',label:'発言者',def:'ある成功者の言葉'},{key:'person',label:'人物 右(任意)',type:'file',def:''}],
    render:(d,t)=>{const photo=d.photo?('background:url('+d.photo+') center/cover no-repeat;'):('background:radial-gradient(circle at 45% 45%, #2a2a32, #0e0e12);');
      return '<div style="width:1280px;height:720px;box-sizing:border-box;position:relative;overflow:hidden;display:flex;flex-direction:column;justify-content:center;align-items:flex-start;padding:70px;font-family:'+t.head+';color:#fff;'+photo+'">'
        +(d.photo?'<div style="position:absolute;inset:0;background:rgba(0,0,0,.45);"></div>':'')
        +(d.person?'<img src="'+d.person+'" style="position:absolute;right:0;bottom:0;width:400px;height:auto;"/>':'')
        +'<div style="position:absolute;left:54px;top:80px;display:flex;font-size:200px;font-weight:900;color:'+t.accent+';line-height:.6;">“</div>'
        +'<div style="position:relative;display:flex;flex-direction:column;font-size:104px;font-weight:900;line-height:1.2;margin-top:60px;text-shadow:0 2px 12px rgba(0,0,0,.6);">'+nl(d.quote)+'</div>'
        +'<div style="position:relative;display:flex;margin-top:20px;font-size:40px;font-weight:700;color:#cfcfcf;">— '+d.name+'</div>'
      +'</div>';} },
  { id:'yt_blackboard', name:'Y50 黒板・教育', cat:'アーカイブ', fmt:'youtube',
    fields:[{key:'title',label:'タイトル（白チョーク・改行可）',def:'中学数学\n総まとめ'},{key:'sub',label:'サブ',def:'これだけ覚えればOK'},{key:'items',label:'チップ（改行・最大3）',def:'方程式\n関数\n図形'}],
    render:(d,t)=>{const ch=String(d.items).split('\n').filter(function(x){return x.length;}).slice(0,3).map(function(x){return '<div style="display:flex;border:3px solid #fff;color:#fff;font-size:36px;font-weight:900;padding:6px 20px;border-radius:8px;margin-right:14px;">'+x+'</div>';}).join('');
      return '<div style="width:1280px;height:720px;box-sizing:border-box;position:relative;overflow:hidden;display:flex;flex-direction:column;justify-content:center;align-items:flex-start;padding:80px;font-family:'+t.head+';color:#fff;background:#2e4636;border:24px solid #8a6a44;">'
        +'<div style="position:relative;display:flex;flex-direction:column;font-family:Yomogi;font-size:130px;font-weight:700;line-height:1.1;">'+nl(d.title)+'</div>'
        +'<div style="position:relative;display:flex;width:420px;height:6px;background:#FFE24A;margin:14px 0 22px;border-radius:3px;"></div>'
        +'<div style="position:relative;display:flex;font-size:48px;font-weight:700;margin-bottom:20px;">'+d.sub+'</div>'
        +'<div style="position:relative;display:flex;">'+ch+'</div>'
      +'</div>';} },

  // ===== note 見出し画像（1280×670）※参考画像が来たら本実装。現状は仮 =====
  // ===== ▼ note 実画像1:1再現（Desktop/noteサムネ）▼ =====
  { id:'note_b01', name:'note 写真＋手書き注釈散らし', cat:'サムネ', fmt:'note',
    fields:[
      {key:'img',label:'背景 写真(任意)',type:'file',def:''},
      {key:'n1',label:'注釈 左上（改行可）',def:'アイスラテ\nたっぷりの氷で\nほっと、ひといき。'},
      {key:'n2',label:'注釈 右上（改行可）',def:'アイスティー\nひやしすぎて\nリフレッシュ'},
    ...
    render:(d,t)=>{const bg=d.img?`background:url(${d.img}) center/cover;`:'background:linear-gradient(150deg,#caa978,#7c5a3a);';
      ...
      return `<div style="width:1920px;height:1006px;box-sizing:border-box;position:relative;overflow:hidden;display:flex;${bg}">`
        ...
        +note(d.n3,'bottom:130px;left:100px;font-size:50px;')
  ...
  { id:'note_b07', name:'note 人物写真＋中央特大フチ＋下帯', cat:'サムネ', fmt:'note',
    fields:[
      {key:'img',label:'背景 人物写真(任意)',type:'file',def:''},
      {key:'small',label:'上 小見出し',def:'現役女性管理職'},
      {key:'big',label:'中央 特大',def:'25年人事'},
  ...
  { id:'note_b08', name:'note 後藤達也 紺コラム（金数字）', cat:'サムネ', fmt:'note',
    fields:[
      {key:'brand',label:'上バー 左',def:'後藤達也 note'},
      {key:'tag',label:'上バー タグ',def:'コアメンバーコラム'},
      {key:'l1',label:'見出し1',def:'出世したくない時代'},
      {key:'l2',label:'見出し2（《》金）',def:'管理職希望 《15.7%》'},
      {key:'date',label:'右下 日付',def:'2026年4月26日'}],
    render:(d,t)=>{const gold=function(s){return String(s).replace(/《([^》]*)》/g,'<span style="display:flex;color:#e8b84a;font-size:118px;">$1</span>');};
  ...
  { id:'note_b09', name:'note 後藤達也 明コラム（キーワード色分け3行）', cat:'サムネ', fmt:'note',
    fields:[
      {key:'brand',label:'上バー 左',def:'後藤達也 note'},
      {key:'tag',label:'上バー タグ',def:'ショートコラム'},
      {key:'l1',label:'1行（【】紺ピル/《》緑/〈〉赤）',def:'【Switch2】 値上げ'},
      {key:'l2',label:'2行',def:'《AI投資の余波》 広がる'},
      {key:'l3',label:'3行',def:'Nintendo 〈株低迷〉'},
      {key:'date',label:'右下 日付',def:'2026年5月8日'}],
    render:(d,t)=>{const mk=function(s){return String(s)
        .replace(/【([^】]*)】/g,'<span style="display:flex;background:#16294f;color:#fff;border-radius:18px;padding:4px 26px;">$1</span>')
        .replace(/《([^》]*)》/g,'<span style="display:flex;color:#1a8a7a;">$1</span>')
        .replace(/〈([^〉]*)〉/g,'<span style="display:flex;color:#d83a2a;">$1</span>');};
      const line=function(s){return `<div style="display:flex;align-items:center;justify-content:center;font-size:96px;font-weight:900;color:#16294f;margin:6px 0;gap:14px;">${mk(s)}</div>`;};
      return `<div style="width:1920px;height:1006px;box-sizing:border-box;position:relative;overflow:hidden;display:flex;flex-direction:column;justify-content:center;align-items:center;background:linear-gradient(160deg,#f6f1e4,#e9e1cd);font-family:${t.head};">`
        +`<div style="position:absolute;top:0;left:0;width:100%;display:flex;align-items:center;justify-content:center;background:#16294f;padding:16px 0;"><div style="display:flex;font-size:46px;font-weight:900;color:#fff;">${d.brand}</div><div style="display:flex;color:#e8b84a;margin:0 18px;font-size:40px;">｜</div><div style="display:flex;font-size:42px;font-weight:700;color:#e8b84a;">${d.tag}</div></div>`
        +line(d.l1)+line(d.l2)+line(d.l3)
        +`<div style="position:absolute;right:50px;bottom:40px;display:flex;background:#16294f;color:#fff;font-size:40px;font-weight:700;padding:8px 24px;border-radius:10px;">${d.date}</div>`
      +`</div>`;} },

  { id:'note_b10', name:'note 解析グラフ背景＋下ボタン', cat:'サムネ', fmt:'note',
    fields:[
      {key:'img',label:'背景 グラフ画像(任意)',type:'file',def:''},
      {key:'tag',label:'上 ラベル',def:'【ロト6・ミニロト解析】'},
      {key:'big',label:'メイン（改行可）',def:'全米が驚いた\n「ゲイル理論」の真実'},
      {key:'sub',label:'サブ',def:'運任せは卒業！統計学で挑む数字選び'},
  ...
  { id:'note_b11', name:'note ダーク背景＋右特大フチ文字', cat:'サムネ', fmt:'note',
    fields:[
  ...
  { id:'note_b13', name:'note ダークゲーム＋特大2行(白/赤)', cat:'サムネ', fmt:'note',
    fields:[
      {key:'img',label:'背景 画像(任意・暗め)',type:'file',def:''},
      {key:'big',label:'特大（《》赤・改行可）',def:'食材を\n《作りだせ》'}],
    render:(d,t)=>{const bg=d.img?`background:url(${d.img}) center/cover;`:'background:linear-gradient(120deg,#1a1a1a,#3a2a2a);';
  ...
  { id:'note_b15', name:'note 人物写真＋特大名前＋斜めロゴ', cat:'サムネ', fmt:'note',
    fields:[
      {key:'img',label:'背景 人物写真(任意)',type:'file',def:''},
      {key:'brand',label:'右上 斜めロゴ',def:'Daily Portal Z'},
      {key:'sub',label:'中央上 小',def:'むかない安藤→高校教師'},
  ...
  { id:'note_b17', name:'note 淡色＋手書き装飾＋3段(青強調)', cat:'サムネ', fmt:'note',
    fields:[
      {key:'l1',label:'上',def:'介護職の姉が'},
      {key:'l2',label:'中 大（青）',def:'IT企業に転職した話'},
      {key:'l3',label:'下',def:'〜求人探し編〜'}],
    render:(d,t)=>{const tick=(css)=>`<div style="position:absolute;display:flex;width:10px;height:48px;background:#2a7ad8;border-radius:6px;${css}"></div>`;
      return `<div style="width:1920px;height:1006px;box-sizing:border-box;position:relative;overflow:hidden;display:flex;flex-direction:column;justify-content:center;align-items:center;background:linear-gradient(160deg,#d8efe9,#c2e6dd);font-family:${t.head};">`
        +`<div style="display:flex;font-size:78px;font-weight:900;color:#223344;">${d.l1}</div>`
        +`<div style="display:flex;font-size:118px;font-weight:900;color:#2a7ad8;margin:16px 0;">${d.l2}</div>`
        +`<div style="display:flex;font-size:60px;font-weight:700;color:#3a5560;">${d.l3}</div>`
        +tick('top:300px;left:360px;transform:rotate(20deg);')+tick('top:280px;right:380px;transform:rotate(-20deg);')+tick('bottom:300px;left:420px;transform:rotate(-15deg);')+tick('bottom:300px;right:430px;transform:rotate(15deg);')
      +`</div>`;} },

  { id:'note_b20', name:'note Gemini 写真＋左白文字＋4色ライン', cat:'サムネ', fmt:'note',
    fields:[
      {key:'img',label:'背景 写真(任意)',type:'file',def:''},
      {key:'brand',label:'左上 ブランド',def:'Google Gemini'},
      {key:'big',label:'左 大文字（改行可）',def:'Geminiと\n新しい読書の\n楽しみ方を'}],
    render:(d,t)=>{const bg=d.img?`background:url(${d.img}) center/cover;`:'background:linear-gradient(160deg,#e8e4dc,#cfc8bc);';
      return `<div style="width:1920px;height:1006px;box-sizing:border-box;position:relative;overflow:hidden;display:flex;flex-direction:column;justify-content:center;padding-left:110px;${bg}">`
        +`<div style="position:absolute;top:54px;left:110px;display:flex;align-items:center;"><div style="display:flex;width:34px;height:34px;background:#fff;transform:rotate(45deg);margin-right:24px;"></div><div style="display:flex;font-size:54px;font-weight:700;color:#fff;text-shadow:0 2px 10px rgba(0,0,0,0.45);">${d.brand}</div></div>`
        +`<div style="display:flex;flex-direction:column;font-size:118px;font-weight:900;color:#fff;line-height:1.16;text-shadow:0 3px 16px rgba(0,0,0,0.5);">${nl(d.big)}</div>`
        +`<div style="position:absolute;left:0;bottom:0;width:100%;display:flex;height:16px;"><div style="display:flex;width:25%;background:#4285F4;"></div><div style="display:flex;width:25%;background:#EA4335;"></div><div style="display:flex;width:25%;background:#FBBC05;"></div><div style="display:flex;width:25%;background:#34A853;"></div></div>`
      +`</div>`;} },

  { id:'note_b21', name:'note レッスン型（キャラ＋紫＋話数＋下帯＋タグ）', cat:'サムネ', fmt:'note',
    fields:[
      {key:'img',label:'背景/キャラ 画像(任意)',type:'file',def:''},
      {key:'topband',label:'左上 小帯',def:'ことばの森のふしぎレッスン'},
      {key:'ep',label:'右上 話数',def:'第16話'},
      {key:'big',label:'中央（《》黄・改行可）',def:'読者の記憶に残る\n《“比喩”》の作り方'},
      {key:'band',label:'下 青帯',def:'ブラック・アウルの例えツッコミ講座'},
      {key:'tags',label:'下タグ（カンマ区切り）',def:'例えで伝わる！,イメージで残る！,比喩で心を動かす！'}],
    render:(d,t)=>{const bg=d.img?`background:url(${d.img}) center/cover;`:'background:linear-gradient(150deg,#5a3a8a,#3a2466);';
      const mk=function(s){return String(s).replace(/《([^》]*)》/g,'<span style="display:flex;color:#FFE24A;">$1</span>');};
      const lines=String(d.big).split('\n').map(function(l){return `<div style="display:flex;">${mk(l)}</div>`;}).join('');
  ...
  { id:'note_b25', name:'note ダーク人物＋上品見出し＋下5列メニュー', cat:'サムネ', fmt:'note',
    fields:[
      {key:'img',label:'背景 人物写真(任意・暗め)',type:'file',def:''},
      {key:'small',label:'上 小',def:'構造を理解し、未来を設計する。'},
      {key:'big',label:'中央（《》橙）',def:'戦略で《結果》は変わる'},
      {key:'sub',label:'下 小',def:'教育×キャリアの戦略研究室'},
      {key:'menu',label:'下5列（カンマ区切り）',def:'就活分析,深海魚,中学受験,総合型選抜,教育業界の構造分析'}],
    render:(d,t)=>{const bg=d.img?`background:url(${d.img}) center/cover;`:'background:linear-gradient(120deg,#1a2230,#0e141e);';
      const mk=function(s){return String(s).replace(/《([^》]*)》/g,'<span style="display:flex;color:#e89030;">$1</span>');};
      const menu=String(d.menu).split(',').map(function(x,i){return `<div style="display:flex;flex:1;justify-content:center;align-items:center;padding:0 10px;font-size:34px;font-weight:700;color:#e8ecf2;${i>0?'border-left:1px solid rgba(255,255,255,0.28);':''}">${x}</div>`;}).join('');
      return `<div style="width:1920px;height:1006px;box-sizing:border-box;position:relative;overflow:hidden;display:flex;flex-direction:column;justify-content:center;align-items:center;${bg}">`
        +`<div style="display:flex;font-size:42px;font-weight:500;letter-spacing:6px;color:#dfe6ee;margin-bottom:14px;">${d.small}</div>`
        +`<div style="display:flex;width:1400px;height:2px;background:rgba(255,255,255,0.35);"></div>`
        +`<div style="display:flex;align-items:center;font-size:130px;font-weight:900;color:#fff;margin:22px 0;">${mk(d.big)}</div>`
        +`<div style="display:flex;width:1400px;height:2px;background:rgba(255,255,255,0.35);"></div>`
        +`<div style="display:flex;font-size:44px;font-weight:700;color:#dfe6ee;margin-top:16px;">${d.sub}</div>`
        +`<div style="position:absolute;left:0;bottom:46px;width:100%;display:flex;align-items:center;">${menu}</div>`
      +`</div>`;} },

  { id:'note_b29', name:'note クリーム＋イラスト＋はじめてのnote', cat:'サムネ', fmt:'note',
    fields:[
      {key:'img',label:'右 イラスト(任意)',type:'file',def:''},
      {key:'small',label:'上 手書き小',def:'はじめての'},
      {key:'big',label:'特大（丸ゴシック）',def:'note'}],
    render:(d,t)=>{const ill=d.img?`<div style="position:absolute;right:90px;top:50%;transform:translateY(-50%);width:760px;height:620px;background:url(${d.img}) center/contain no-repeat;display:flex;"></div>`:'';
      return `<div style="width:1920px;height:1006px;box-sizing:border-box;position:relative;overflow:hidden;display:flex;flex-direction:column;justify-content:center;padding-left:170px;background:linear-gradient(160deg,#efe3c8,#e6d6b4);">`
        +`<div style="display:flex;font-family:Yomogi;font-size:96px;color:#2a2a2a;margin-bottom:6px;">${d.small}</div>`
        +`<div style="display:flex;font-family:'Zen Maru Gothic';font-size:300px;font-weight:900;color:#1a1a1a;line-height:0.9;">${d.big}</div>`
        +ill
      +`</div>`;} },

  { id:'note_b32', name:'note 風景写真＋特大2行＋下数字3box', cat:'サムネ', fmt:'note',
    fields:[
      {key:'img',label:'背景 写真(任意)',type:'file',def:''},
      {key:'top',label:'上 帯テキスト',def:'Note初心者が、50日毎日書いて気づいた。'},
      {key:'big',label:'中央（《》黄・改行可）',def:'自己紹介が\n《一番難しい。》'},
      {key:'boxes',label:'下3box（ラベル|値 を / で区切る）',def:'甲子園|3回/転職|4回/相談|1,200件'}],
    render:(d,t)=>{const bg=d.img?`background:url(${d.img}) center/cover;`:'background:linear-gradient(160deg,#3a4a5a,#1a2530);';
      const mk=function(s){return String(s).replace(/《([^》]*)》/g,'<span style="display:flex;color:#FFE24A;">$1</span>');};
      const lines=String(d.big).split('\n').map(function(l){return `<div style="display:flex;justify-content:center;">${mk(l)}</div>`;}).join('');
  ...
  { id:'note_b37', name:'note AI成長ストーリー（吹き出し＋下ステップ）', cat:'サムネ', fmt:'note',
    fields:[
      {key:'img',label:'背景 写真(任意)',type:'file',def:''},
      {key:'top',label:'上 ラベル',def:'AI時代の成長ストーリー'},
      {key:'quote',label:'中央 吹き出し',def:'『アトカさん、AI副業 僕にもできますか?』'},
      {key:'sub',label:'下 説明',def:'普通の会社員・23歳の僕が、毎日10分の積み上げで人生を変えていく物語。'},
      {key:'steps',label:'下ステップ（カンマ区切り）',def:'毎日10分の継続,AIで効率化,小さな前進,未来を変える'}],
    render:(d,t)=>{const bg=d.img?`background:url(${d.img}) center/cover;`:'background:linear-gradient(160deg,#eaf1f8,#cfe0ee);';
      const steps=String(d.steps).split(',').map(function(x){return `<div style="display:flex;align-items:center;justify-content:center;background:#1763c4;color:#fff;font-size:34px;font-weight:900;padding:12px 28px;margin:0 10px;border-radius:12px;">${x}</div>`;}).join('');
      return `<div style="width:1920px;height:1006px;box-sizing:border-box;position:relative;overflow:hidden;display:flex;flex-direction:column;justify-content:center;align-items:center;${bg}">`
        +`<div style="display:flex;background:#16335c;color:#fff;font-size:42px;font-weight:900;padding:10px 34px;border-radius:30px;margin-bottom:26px;">${d.top}</div>`
        +`<div style="display:flex;align-items:center;justify-content:center;background:#fff;border:8px solid #1763c4;border-radius:24px;padding:30px 56px;max-width:1500px;"><div style="display:flex;font-size:78px;font-weight:900;color:#16335c;text-align:center;line-height:1.25;">${d.quote}</div></div>`
        +`<div style="display:flex;font-size:40px;font-weight:700;color:#1a3050;margin-top:24px;">${d.sub}</div>`
        +`<div style="display:flex;margin-top:26px;">${steps}</div>`
      +`</div>`;} },

  { id:'note_b38', name:'note 白背景＋右人物＋左赤特大袋文字', cat:'サムネ', fmt:'note',
    fields:[
  ...
  { id:'note_b48', name:'note 4枠グリッド(2×2)＋上タイトル', cat:'サムネ', fmt:'note',
    fields:[
  ...
  { id:'note_b50', name:'note テクスチャ背景＋中央 手書き白', cat:'サムネ', fmt:'note',
    fields:[
      {key:'img',label:'背景 テクスチャ(任意)',type:'file',def:''},
      {key:'title',label:'中央 手書き',def:'自己紹介'}],
    render:(d,t)=>{const bg=d.img?`background:url(${d.img}) center/cover;`:'background:linear-gradient(160deg,#cd9a5a,#b07e40);';
      return `<div style="width:1920px;height:1006px;box-sizing:border-box;position:relative;overflow:hidden;display:flex;justify-content:center;align-items:center;${bg}">`
        +`<div style="display:flex;font-family:Yomogi;font-size:170px;font-weight:700;letter-spacing:30px;color:#fff;text-shadow:0 3px 12px rgba(0,0,0,0.3);">${d.title}</div>`
      +`</div>`;} },

  { id:'note_b51', name:'note 明朝・上品サイトマップ型', cat:'サムネ', fmt:'note',
    fields:[
      {key:'img',label:'背景 写真(任意)',type:'file',def:''},
      {key:'small',label:'上 小（下線）',def:'このアカウントの'},
      {key:'big',label:'明朝 特大',def:'サイトマップ'},
      {key:'en',label:'英字',def:'- Sitemap -'},
      {key:'band',label:'グレー帯',def:'記事の全体像が一目でわかる'},
      {key:'sub',label:'下 小',def:'留学・英語学習・リアルな体験を発信中'}],
    render:(d,t)=>{const bg=d.img?`background:url(${d.img}) center/cover;`:'background:linear-gradient(160deg,#efece4,#e2ddd1);';
      return `<div style="width:1920px;height:1006px;box-sizing:border-box;position:relative;overflow:hidden;display:flex;flex-direction:column;justify-content:center;align-items:flex-start;padding-left:120px;font-family:'Shippori Mincho';color:#3a352e;${bg}">`
        +`<div style="display:flex;font-size:56px;border-bottom:3px solid #3a352e;padding-bottom:14px;">${d.small}</div>`
        +`<div style="display:flex;font-size:158px;font-weight:700;margin:18px 0 6px;">${d.big}</div>`
        +`<div style="display:flex;font-size:50px;letter-spacing:8px;color:#6a6358;">${d.en}</div>`
        +`<div style="display:flex;background:#8a8176;color:#fff;font-size:52px;padding:12px 34px;margin:30px 0 16px;">${d.band}</div>`
        +`<div style="display:flex;font-size:42px;color:#5a544a;">${d.sub}</div>`
      +`</div>`;} },

  { id:'note_b54', name:'note 人物夜景＋左小＋右多色＋下帯', cat:'サムネ', fmt:'note',
    fields:[
      {key:'img',label:'背景 写真(任意・暗め)',type:'file',def:''},
      {key:'small',label:'左上 小',def:'僕を表す色は、黒です。'},
      {key:'multi',label:'右上（《》黄/〈〉水・改行可）',def:'あなたを《色》で表すと\n《何色》ですか?\nその理由は?'},
      {key:'band',label:'下帯',def:'【本田健】ハッピーライティングマラソン'}],
    render:(d,t)=>{const bg=d.img?`background:url(${d.img}) center/cover;`:'background:linear-gradient(120deg,#2a3040,#101620);';
  ...
  { id:'note_b65', name:'note 桃色＋イラスト＋はじめてのnote(袋文字)', cat:'サムネ', fmt:'note',
    fields:[
      {key:'img',label:'右 イラスト(任意)',type:'file',def:''},
      {key:'small',label:'上 小',def:'はじめての'},
      {key:'big',label:'特大（丸ゴシック袋文字）',def:'note'}],
  ...
  { id:'note_b66', name:'note 紺フィンテック＋白特大＋赤サブ', cat:'サムネ', fmt:'note',
    fields:[
      {key:'title',label:'中央 特大',def:'消えた仮想通貨JCT'},
      {key:'sub',label:'下 赤サブ',def:'12人の公式メンバー。東証上場企業2社。全員が沈黙。'}],
    render:(d,t)=>`<div style="width:1920px;height:1006px;box-sizing:border-box;position:relative;overflow:hidden;display:flex;flex-direction:column;justify-content:center;align-items:center;background:radial-gradient(circle at 50% 40%,#22345c,#0c1428);font-family:${t.head};">`
        +`<div style="display:flex;font-size:158px;font-weight:900;color:#fff;text-shadow:0 4px 20px rgba(0,0,0,0.5);">${d.title}</div>`
        +`<div style="display:flex;font-size:54px;font-weight:900;color:#e8463a;margin-top:24px;">${d.sub}</div>`
      +`</div>` },

  { id:'note_b68', name:'note 黒＋炎＋毛筆オレンジ', cat:'サムネ', fmt:'note',
    fields:[
      {key:'pre',label:'上',def:'堀元見の'},
      {key:'big',label:'中央 大（炎）',def:'炎上'},
      {key:'tail',label:'大の続き',def:'するから'},
      {key:'bottom',label:'右下',def:'有料で書く話'}],
    render:(d,t)=>`<div style="width:1920px;height:1006px;box-sizing:border-box;position:relative;overflow:hidden;display:flex;flex-direction:column;justify-content:center;align-items:center;background:radial-gradient(circle at 50% 52%,#3a1604,#000 70%);font-family:Yomogi;">`
        +`<div style="display:flex;align-self:flex-start;margin-left:120px;font-size:84px;font-weight:700;color:#ff5a1a;text-shadow:0 0 24px #ff8a2a;">${d.pre}</div>`
        +`<div style="display:flex;align-items:flex-end;"><div style="display:flex;font-size:210px;font-weight:700;color:#ffb020;text-shadow:0 0 40px #ff6a10,0 0 70px #ff4a00;">${d.big}</div><div style="display:flex;font-size:90px;font-weight:700;color:#ff5a1a;text-shadow:0 0 24px #ff8a2a;margin-bottom:30px;">${d.tail}</div></div>`
  ...
  { id:'note_b71', name:'note 明朝・淡写真＋上ラベル＋中央2行', cat:'サムネ', fmt:'note',
    fields:[
      {key:'img',label:'背景 写真(任意)',type:'file',def:''},
      {key:'label',label:'上 ラベル（下線）',def:'はじめてのnote'},
      {key:'lines',label:'中央（改行可）',def:'限界ワーママ、\n会社員を手放して起業に挑戦します'}],
    render:(d,t)=>{const bg=d.img?`background:url(${d.img}) center/cover;`:'background:linear-gradient(160deg,#f4efe6,#eae2d2);';
      return `<div style="width:1920px;height:1006px;box-sizing:border-box;position:relative;overflow:hidden;display:flex;flex-direction:column;justify-content:center;align-items:flex-start;padding-left:120px;font-family:'Shippori Mincho';color:#3a352e;${bg}">`
        +`<div style="display:flex;font-size:48px;letter-spacing:6px;border-bottom:2px solid #3a352e;padding-bottom:10px;margin-bottom:34px;">${d.label}</div>`
  ...
  { id:'note_b74', name:'note キラ背景＋角括弧ラベル＋中央3行（マラソン型）', cat:'サムネ', fmt:'note',
    fields:[
      {key:'img',label:'背景 キラ画像(任意)',type:'file',def:''},
      {key:'label',label:'上 枠ラベル',def:'ハッピーライティングマラソン#49'},
      {key:'lines',label:'中央（改行可）',def:'もし明日、\n素敵な偶然が起きると\nしたら何が起きそう？'}],
    render:(d,t)=>{const bg=d.img?`background:url(${d.img}) center/cover;`:'background:linear-gradient(135deg,#f1e6cc,#e6d6e8);';
      return `<div style="width:1920px;height:1006px;box-sizing:border-box;position:relative;overflow:hidden;display:flex;flex-direction:column;justify-content:center;align-items:center;${bg}">`
        +`<div style="display:flex;border:3px solid #6a5a3a;padding:10px 34px;color:#5a4a2a;font-size:40px;font-weight:700;margin-bottom:30px;">${d.label}</div>`
  ...
  { id:'note_s04', name:'note ネオンLP型（キャラ＋上帯＋3行＋下5チップ）', cat:'サムネ', fmt:'note',
    fields:[
      {key:'img',label:'背景 ネオン画像(任意)',type:'file',def:''},
      {key:'top',label:'上 黄帯',def:'売れないのは才能じゃない、原理原則を知らないだけ。'},
      {key:'big',label:'中央3行（《》桃・改行可）',def:'人間の《本性》を操り、\n《モノを売る》\n究極の《心理戦略》'},
      {key:'menu',label:'中段メニュー',def:'本質・戦略・設計・嚢導・成約'},
      {key:'chips',label:'下5チップ（カンマ区切り）',def:'人の欲求を刺激,顧客心理を見抜く,信頼を高める,心を動かす文章,行動を加速'}],
    render:(d,t)=>{const bg=d.img?`background:url(${d.img}) center/cover;`:'background:radial-gradient(circle at 50% 40%,#3a1a6a,#0e0a26);';
      const mk=function(s){return String(s).replace(/《([^》]*)》/g,'<span style="display:flex;color:#ff4aa0;">$1</span>');};
      const lines=String(d.big).split('\n').map(function(l){return `<div style="display:flex;justify-content:center;">${mk(l)}</div>`;}).join('');
  ...
  { id:'note_s06', name:'note NEWS分析型（写真＋3段テロップ＋右下バッジ）', cat:'サムネ', fmt:'note',
    fields:[
      {key:'img',label:'背景 写真(任意・暗め)',type:'file',def:''},
      {key:'l1',label:'1段目',def:'AIに「書くこと」を'},
      {key:'l2',label:'2段目（白帯・赤字）',def:'奪われそうな夜に読むnote'},
      {key:'l3',label:'3段目',def:'あなたの文章は「下手」じゃない'},
  ...
  { id:'note_s11', name:'note ド派手AI型（多色袋文字3段）', cat:'サムネ', fmt:'note',
    fields:[
      {key:'img',label:'背景 画像(任意・派手)',type:'file',def:''},
      {key:'top',label:'上（《》赤）',def:'これは画像生成の《革命》か?!'},
      {key:'mid',label:'中（《》水色）',def:'ついに《ChatGPT》が'},
      {key:'big',label:'下 特大（《》黄）',def:'《本気》を出した'}],
  ...
  { id:'note_s13', name:'note 暖色＋上 赤袋文字＋下 褒め言葉ふきだし', cat:'サムネ', fmt:'note',
    fields:[
      {key:'img',label:'背景 イラスト(任意)',type:'file',def:''},
      {key:'lines',label:'上 2行（赤袋文字・改行可）',def:'「親バカ」は最高の\n子育て戦略である'},
      {key:'bubbles',label:'下 褒め言葉（カンマ区切り）',def:'すごいね,だいすき,よくできたね,さいこう！'}],
    render:(d,t)=>{const bg=d.img?`background:url(${d.img}) center/cover;`:'background:linear-gradient(160deg,#fde7d2,#f6cca8);';
      const lines=String(d.lines).split('\n').map(function(l){return `<div style="display:flex;justify-content:center;">${l}</div>`;}).join('');
      const cols=['#ff8a3a','#ff6f7a','#5ab0e0','#7ec46a'];
      const bb=String(d.bubbles).split(',').map(function(x,i){return `<div style="display:flex;align-items:center;justify-content:center;background:#fff;color:${cols[i%cols.length]};font-family:Yomogi;font-size:48px;font-weight:700;padding:16px 38px;border-radius:50px;margin:0 14px;box-shadow:0 8px 20px rgba(180,110,70,0.2);">${x}</div>`;}).join('');
      return `<div style="width:1920px;height:1006px;box-sizing:border-box;position:relative;overflow:hidden;display:flex;flex-direction:column;justify-content:space-between;align-items:center;padding:120px 90px 130px;${bg}">`
        +`<div style="display:flex;flex-direction:column;align-items:center;font-size:124px;font-weight:900;color:#e8341e;line-height:1.2;${xstroke(11,'#ffffff')}">${lines}</div>`
        +`<div style="display:flex;flex-wrap:wrap;justify-content:center;">${bb}</div>`
      +`</div>`;} },

  { id:'note_s14', name:'note 金炎LP＋特大英字＋下チップ', cat:'サムネ', fmt:'note',
    fields:[
      {key:'img',label:'背景 画像(任意・金炎)',type:'file',def:''},
      {key:'top',label:'上 帯',def:'Xで爆発的に伸ばすための教科書'},
      {key:'big',label:'特大タイトル',def:'Xplosion'},
      {key:'sub',label:'下 サブ',def:'X運用82日で結果を出した、すべてを公開'},
      {key:'chips',label:'下チップ（カンマ区切り）',def:'フォロワー1万人,2ヶ月で収益化,完全有料,44日で60万円'}],
    render:(d,t)=>{const bg=d.img?`background:url(${d.img}) center/cover;`:'background:radial-gradient(circle at 50% 45%,#5a3400,#0a0600 75%);';
  ...
  { id:'note_s15', name:'note 黒金神秘＋上小＋金2行＋下赤帯', cat:'サムネ', fmt:'note',
    fields:[
      {key:'img',label:'背景 画像(任意・黒金)',type:'file',def:''},
      {key:'top',label:'上 小',def:'7年間の研究で辿り着いた'},
      {key:'big',label:'金 特大（改行可）',def:'引き寄せの法則\n最短発動ワーク'},
      {key:'band',label:'下 赤帯',def:'※いきなり叶い始めます　覚悟して実践してください'}],
    render:(d,t)=>{const bg=d.img?`background:url(${d.img}) center/cover;`:'background:radial-gradient(circle at 50% 42%,#4a3a10,#080604 72%);';
      return `<div style="width:1920px;height:1006px;box-sizing:border-box;position:relative;overflow:hidden;display:flex;flex-direction:column;${bg}">`
        +`<div style="flex:1;display:flex;flex-direction:column;justify-content:center;align-items:center;">`
          +`<div style="display:flex;font-size:48px;font-weight:700;letter-spacing:4px;color:#f0e0b0;margin-bottom:20px;">${d.top}</div>`
          +`<div style="display:flex;flex-direction:column;align-items:center;font-size:128px;font-weight:900;color:#e8c860;line-height:1.2;text-shadow:0 0 26px rgba(220,180,80,0.6);">${nl(d.big)}</div>`
        +`</div>`
        +`<div style="width:100%;display:flex;justify-content:center;align-items:center;background:#b81818;padding:24px 0;"><div style="display:flex;font-size:42px;font-weight:900;color:#fff;">${d.band}</div></div>`
      +`</div>`;} },

  { id:'note_s16', name:'note 保存版まとめパネル（数字＋画像は任意）', cat:'サムネ', fmt:'note',
    fields:[
      {key:'img',label:'左 イラスト(任意)',type:'file',def:''},
      {key:'badge',label:'黄バッジ',def:'保存版'},
      {key:'big',label:'白パネル内（改行可）',def:'冷蔵庫に貼るだけ\nレシピ帳'},
      {key:'num',label:'丸 数字',def:'108'},
      {key:'unit',label:'丸 単位',def:'レシピ'}],
    render:(d,t)=>{const ill=d.img?`<div style="position:absolute;left:70px;top:50%;transform:translateY(-50%);width:520px;height:680px;background:url(${d.img}) center/contain no-repeat;display:flex;"></div>`:'';
      return `<div style="width:1920px;height:1006px;box-sizing:border-box;position:relative;overflow:hidden;display:flex;justify-content:${d.img?'flex-end':'center'};align-items:center;padding:0 90px;background:#28c0dc;">`
        +ill
  ...
  { id:'note_s17', name:'note アニメ美女＋紫＋特大白袋文字＋ピンク帯', cat:'サムネ', fmt:'note',
    fields:[
      {key:'img',label:'右 美女(任意)',type:'file',def:''},
      {key:'top',label:'上 小',def:'知識0 実績0でも 即収益'},
      {key:'big',label:'特大（白袋文字）',def:'月結び'},
      {key:'sub',label:'下 サブ',def:'スキマ時間だけで月5〜10万を安定して稼ぐ'},
      {key:'band',label:'ピンク帯',def:'占い副業・完全ガイド'}],
    render:(d,t)=>{const right=d.img?`<div style="position:absolute;right:0;top:0;width:560px;height:1006px;background:url(${d.img}) center/cover;display:flex;"></div>`:'';
  ...
  { id:'note_s21', name:'note 水色＋美女右＋左黒袋文字＋ピンク帯', cat:'サムネ', fmt:'note',
    fields:[
      {key:'img',label:'右 美女(任意)',type:'file',def:''},
      {key:'top',label:'上 小',def:'ロゴもサムネもフォントも、すべて作れる時代へ。'},
      {key:'big',label:'特大（黒袋文字）',def:'GPT image2'},
      {key:'band',label:'ピンク帯',def:'プロンプト集'},
      {key:'sub',label:'下 サブ',def:'アイデアを、かたちにする革命。'},
      {key:'author',label:'著者',def:'竜田みどり'}],
    render:(d,t)=>{const right=d.img?`<div style="position:absolute;right:0;top:0;width:560px;height:1006px;background:url(${d.img}) center/cover;display:flex;"></div>`:'';
  ...
  { id:'note_s23', name:'note 単色＋著者コラム型（アイコン＋note＋中央2行）', cat:'サムネ', fmt:'note',
    fields:[
      {key:'img',label:'左上 アイコン(任意)',type:'file',def:''},
      {key:'author',label:'著者名',def:'Testosterone'},
      {key:'big',label:'中央（改行可）',def:'『地獄に堕ちるわよ』\nなぜ人は占いに大金を払うのか'}],
    render:(d,t)=>{const av=d.img?`background:url(${d.img}) center/cover;`:'background:#9a9a9a;';
      return `<div style="width:1920px;height:1006px;box-sizing:border-box;position:relative;overflow:hidden;display:flex;justify-content:center;align-items:center;background:#c265e0;font-family:${t.head};">`
        +`<div style="position:absolute;top:46px;left:54px;display:flex;align-items:center;"><div style="display:flex;width:96px;height:96px;border-radius:50%;${av}margin-right:24px;"></div><div style="display:flex;font-size:56px;font-weight:900;color:#1a1a1a;">${d.author}</div></div>`
        +`<div style="position:absolute;top:50px;right:60px;display:flex;font-size:64px;font-weight:900;letter-spacing:6px;color:#1a1a1a;">note</div>`
        +`<div style="display:flex;flex-direction:column;align-items:center;font-size:92px;font-weight:900;color:#161616;line-height:1.35;">${nl(d.big)}</div>`
      +`</div>`;} },

  { id:'note_s26', name:'note 暗背景＋右アニメ少女＋左テキスト(数字強調)', cat:'サムネ', fmt:'note',
    fields:[
      {key:'img',label:'右 キャラ(任意)',type:'file',def:''},
      {key:'top',label:'上 小',def:'AIインフラ設備投資'},
      {key:'big',label:'特大',def:'7,000億ドル'},
      {key:'sub',label:'下（〈〉赤）',def:'それでも、〈NVIDIA〉は下落した'},
      {key:'sub2',label:'最下 小',def:'市場の読みと、私が見ている景色は違う'}],
    render:(d,t)=>{const right=d.img?`<div style="position:absolute;right:0;top:0;width:520px;height:1006px;background:url(${d.img}) center/cover;display:flex;"></div>`:'';
      const mk=function(s){return String(s).replace(/〈([^〉]*)〉/g,'<span style="display:flex;color:#ff4a4a;">$1</span>');};
      return `<div style="width:1920px;height:1006px;box-sizing:border-box;position:relative;overflow:hidden;display:flex;flex-direction:column;justify-content:center;align-items:flex-start;padding-left:100px;background:radial-gradient(circle at 35% 40%,#16203a,#06080f 75%);font-family:${t.head};">`
        +right
        +`<div style="position:relative;display:flex;font-size:46px;font-weight:700;color:#cdd6e6;margin-bottom:6px;">${d.top}</div>`
        +`<div style="position:relative;display:flex;font-size:168px;font-weight:900;color:#fff;line-height:1;text-shadow:0 4px 18px rgba(0,0,0,0.5);">${d.big}</div>`
        +`<div style="position:relative;display:flex;align-items:center;font-size:74px;font-weight:900;color:#fff;margin-top:18px;">${mk(d.sub)}</div>`
        +`<div style="position:relative;display:flex;font-size:38px;font-weight:500;color:#aeb8c8;margin-top:14px;">${d.sub2}</div>`
      +`</div>`;} },

  { id:'note_s30', name:'note 黒金＋左アイコン＋角括弧帯＋金2行', cat:'サムネ', fmt:'note',
    fields:[
      {key:'img',label:'左 アイコン(任意)',type:'file',def:''},
      {key:'badge',label:'上 帯ラベル',def:'SNS不要'},
      {key:'big',label:'2行（《》金・改行可）',def:'顔出しなしで\n《稼ぐ副業》'},
      {key:'tail',label:'小',def:'まとめ'},
      {key:'items',label:'下チップ（カンマ区切り）',def:'ブログ,コンテンツ販売,物販,AI活用,動画編集'}],
    render:(d,t)=>{const mk=function(s){return String(s).replace(/《([^》]*)》/g,'<span style="display:flex;color:#e8c860;">$1</span>');};
      const lines=String(d.big).split('\n').map(function(l){return `<div style="display:flex;justify-content:center;">${mk(l)}</div>`;}).join('');
      const chips=String(d.items).split(',').filter(function(x){return x.length;}).map(function(x){return `<div style="display:flex;align-items:center;justify-content:center;border:2px solid #b89a4a;color:#e8d8a0;font-size:38px;font-weight:900;padding:9px 28px;border-radius:50px;margin:8px;">${x}</div>`;}).join('');
      return `<div style="width:1920px;height:1006px;box-sizing:border-box;position:relative;overflow:hidden;display:flex;flex-direction:column;justify-content:center;align-items:center;background:radial-gradient(circle at 55% 42%,#2a2410,#080604 72%);font-family:${t.head};">`
        +`<div style="display:flex;align-items:center;border-top:2px solid #c8a850;border-bottom:2px solid #c8a850;padding:8px 40px;color:#e8d8a0;font-size:44px;font-weight:900;margin-bottom:22px;">${d.badge}</div>`
        +`<div style="display:flex;flex-direction:column;align-items:center;font-size:120px;font-weight:900;color:#fff;line-height:1.14;text-shadow:0 3px 14px rgba(0,0,0,0.5);">${lines}</div>`
        +`<div style="display:flex;font-size:46px;font-weight:900;color:#e8d8a0;margin-top:8px;">${d.tail}</div>`
        +`<div style="display:flex;flex-wrap:wrap;justify-content:center;max-width:1500px;margin-top:40px;">${chips}</div>`
      +`</div>`;} },

  { id:'note_s31', name:'note 金LP＋特大3行＋下帯', cat:'サムネ', fmt:'note',
    fields:[
      {key:'img',label:'背景 画像(任意・金)',type:'file',def:''},
      {key:'big',label:'特大3行（《》金・改行可）',def:'ChatGPT\n《画像生成》\n《オールインパック》'},
      {key:'sub',label:'下 帯',def:'SNS・広告・バナー・図解・漫画・表紙までプロ品質でワンストップで実現！'}],
    render:(d,t)=>{const bg=d.img?`background:url(${d.img}) center/cover;`:'background:radial-gradient(circle at 50% 45%,#5a3e08,#0c0804 78%);';
      const mk=function(s){return String(s).replace(/《([^》]*)》/g,'<span style="display:flex;color:#ffd24a;">$1</span>');};
      const lines=String(d.big).split('\n').map(function(l){return `<div style="display:flex;justify-content:center;">${mk(l)}</div>`;}).join('');
      return `<div style="width:1920px;height:1006px;box-sizing:border-box;position:relative;overflow:hidden;display:flex;flex-direction:column;justify-content:center;align-items:center;${bg}">`
  ...
  { id:'note_s33', name:'note ダーク＋毛筆フレーズ（《》赤・自由改行）', cat:'サムネ', fmt:'note',
    fields:[
      {key:'img',label:'背景 写真(任意)',type:'file',def:''},
      {key:'lines',label:'フレーズ（改行可・《》で赤）',def:'情報を並べただけの\n《有料記事》、だからなんだ?'}],
    render:(d,t)=>{const bg=d.img?`background:url(${d.img}) center/cover;`:'background:linear-gradient(150deg,#3a4658 0%,#232b37 58%,#171c26 100%);';
      const mk=function(s){return String(s).replace(/《([^》]*)》/g,'<span style="display:flex;color:#ff3a26;">$1</span>');};
      const lines=String(d.lines).split('\n').map(function(l){return `<div style="display:flex;align-items:center;justify-content:center;">${mk(l)}</div>`;}).join('');
      return `<div style="width:1920px;height:1006px;box-sizing:border-box;position:relative;overflow:hidden;display:flex;flex-direction:column;justify-content:center;align-items:center;padding:0 120px;${bg}">`
        +`<div style="position:absolute;left:0;top:0;width:100%;height:100%;display:flex;background:linear-gradient(180deg,rgba(0,0,0,0.35),rgba(0,0,0,0.05) 38%,rgba(0,0,0,0.1) 66%,rgba(0,0,0,0.45));"></div>`
        +`<div style="position:absolute;left:120px;top:130px;display:flex;font-family:'Shippori Mincho';font-size:300px;line-height:0.7;color:rgba(255,255,255,0.12);">「</div>`
        +`<div style="position:absolute;right:120px;bottom:120px;display:flex;font-family:'Shippori Mincho';font-size:300px;line-height:0.7;color:rgba(255,255,255,0.12);">」</div>`
        +`<div style="position:relative;display:flex;flex-direction:column;align-items:center;font-family:Yomogi;font-size:122px;font-weight:700;color:#fff;line-height:1.42;${xstroke(10,'#10141c')}">${lines}</div>`
      +`</div>`;} },

  { id:'note_s34', name:'note ミニマル紫＋中央2行＋下 日付/ブランド', cat:'サムネ', fmt:'note',
    fields:[
      {key:'img',label:'背景 画像(任意)',type:'file',def:''},
      {key:'big',label:'中央2行（《》水色・改行可）',def:'Claude CodeとClaude Designで\n完結する新しい《スライド制作術》'},
      {key:'date',label:'左下 日付',def:'Apr 24, 2026'},
      {key:'brand',label:'右下 ブランド',def:'POSTS'}],
    render:(d,t)=>{const bg=d.img?`background:url(${d.img}) center/cover;`:'background:linear-gradient(120deg,#3a3a8a,#5a3a9a,#2a2a5a);';
      const mk=function(s){return String(s).replace(/《([^》]*)》/g,'<span style="display:flex;color:#9ab8ff;">$1</span>');};
      const lines=String(d.big).split('\n').map(function(l){return `<div style="display:flex;justify-content:center;">${mk(l)}</div>`;}).join('');
      return `<div style="width:1920px;height:1006px;box-sizing:border-box;position:relative;overflow:hidden;display:flex;justify-content:center;align-items:center;${bg}">`
        +`<div style="display:flex;flex-direction:column;align-items:center;font-size:76px;font-weight:700;color:#fff;line-height:1.4;">${lines}</div>`
        +`<div style="position:absolute;left:60px;bottom:44px;display:flex;font-size:36px;font-weight:700;color:#c8c8e8;letter-spacing:3px;">${d.date}</div>`
        +`<div style="position:absolute;right:60px;bottom:44px;display:flex;font-size:42px;font-weight:900;color:#fff;letter-spacing:6px;">${d.brand}</div>`
      +`</div>`;} },

  { id:'note_s35', name:'note 人物＋左下テロップ＋右上 小', cat:'サムネ', fmt:'note',
    fields:[
      {key:'img',label:'背景 人物写真(任意)',type:'file',def:''},
      {key:'l1',label:'左下 黄',def:'28卒版'},
      {key:'l2',label:'左下 白',def:'SESには入社させない'},
      {key:'corner',label:'右上 小（改行可）',def:'ホントのこと\n知りたくないのか?'}],
  ...
  { id:'note_s36', name:'note アニメ美女＋左テキスト（強調特大）', cat:'サムネ', fmt:'note',
    fields:[
      {key:'img',label:'右 美女(任意)',type:'file',def:''},
      {key:'big',label:'左（《》特大・改行可）',def:'《彼女》と\n呼ばれないまま、\n《1年》が過ぎた'},
      {key:'sub',label:'下 サブ',def:'―本命に進むか、降りるか、自分で決める'}],
    render:(d,t)=>{const bg=d.img?`background:url(${d.img}) center/cover;`:'background:linear-gradient(120deg,#caa0a8,#7a5060);';
  ...
  { id:'note_s39', name:'note 暗ロマン写真＋中央4段', cat:'サムネ', fmt:'note',
    fields:[
      {key:'img',label:'背景 写真(任意・暗め)',type:'file',def:''},
      {key:'label',label:'上 枠ラベル',def:'【片思い両思い】を'},
      {key:'big',label:'特大',def:'実らせる'},
      {key:'s1',label:'小',def:'簡単な'},
      {key:'s2',label:'下',def:'恋愛上級者テクニック'}],
    render:(d,t)=>{const bg=d.img?`background:url(${d.img}) center/cover;`:'background:linear-gradient(150deg,#3a2e26,#140e0a);';
      return `<div style="width:1920px;height:1006px;box-sizing:border-box;position:relative;overflow:hidden;display:flex;flex-direction:column;justify-content:center;align-items:center;${bg}">`
        +`<div style="display:flex;border:2px solid rgba(255,255,255,0.7);padding:8px 30px;color:#fff;font-size:46px;font-weight:700;margin-bottom:30px;text-shadow:0 2px 8px rgba(0,0,0,0.6);">${d.label}</div>`
        +`<div style="display:flex;font-size:168px;font-weight:900;color:#fff;line-height:1;${xstroke(9,'#1a1a1a','rgba(0,0,0,0)')};">${d.big}</div>`
        +`<div style="display:flex;font-size:48px;font-weight:700;color:#fff;margin-top:30px;text-shadow:0 2px 8px rgba(0,0,0,0.6);">${d.s1}</div>`
        +`<div style="display:flex;font-size:76px;font-weight:900;color:#fff;margin-top:14px;text-shadow:0 2px 10px rgba(0,0,0,0.6);">${d.s2}</div>`
      +`</div>`;} },

  { id:'note_s42', name:'note 金枠＋実録ラベル＋黒特大＋サブ（事件・暴露）', cat:'サムネ', fmt:'note',
    fields:[
      {key:'img',label:'背景 画像(任意)',type:'file',def:''},
      {key:'label',label:'上 ラベル',def:'実録'},
      {key:'big',label:'特大タイトル（改行可）',def:'消えた2000万と\n一通の訴状'},
      {key:'sub',label:'下 サブ',def:'その顛末を、すべて書きます'}],
    render:(d,t)=>{const bg=d.img?`background:url(${d.img}) center/cover;`:'background:#f6edd6;';
      const lines=String(d.big).split('\n').map(function(l){return `<div style="display:flex;">${l}</div>`;}).join('');
      return `<div style="width:1920px;height:1006px;box-sizing:border-box;position:relative;overflow:hidden;display:flex;flex-direction:column;justify-content:center;padding:0 110px;border:18px solid #c9a94e;${bg}">`
        +`<div style="display:flex;align-items:center;margin-bottom:30px;"><div style="display:flex;width:64px;height:6px;background:#c9a94e;margin-right:22px;"></div><div style="display:flex;font-size:48px;font-weight:900;letter-spacing:10px;color:#9a7a2e;">${d.label}</div></div>`
        +`<div style="display:flex;flex-direction:column;font-size:152px;font-weight:900;color:#1a1a1a;line-height:1.16;${xstroke(8,'#f6edd6','rgba(0,0,0,0)')}">${lines}</div>`
        +`<div style="display:flex;font-size:50px;font-weight:700;color:#6a583a;margin-top:30px;">${d.sub}</div>`
      +`</div>`;} },


  { id:'note_s55', name:'note YouTube副業型（ロゴ＋数字袋文字＋帯）', cat:'サムネ', fmt:'note',
    fields:[
      {key:'img',label:'右 キャラ(任意)',type:'file',def:''},
      {key:'badge',label:'バッジ（カテゴリ）',def:'YouTube'},
      {key:'corner',label:'右上 小（任意・空欄で非表示）',def:'顔出しなし・声出しなし'},
      {key:'l1',label:'1行（《》桃）',def:'再生数《0》から'},
      {key:'l2',label:'特大',def:'150万円'},
      {key:'band',label:'帯',def:'ジャンル公開！'},
      {key:'tag',label:'タグ',def:'未経験主婦'}],
    render:(d,t)=>{const right=d.img?`<div style="position:absolute;right:60px;top:50%;transform:translateY(-50%);width:360px;height:360px;background:url(${d.img}) center/contain no-repeat;display:flex;"></div>`:'';
      const corner=String(d.corner||'').trim()?`<div style="position:absolute;right:64px;top:56px;display:flex;background:rgba(26,26,26,0.85);color:#fff;font-size:34px;font-weight:700;padding:8px 24px;border-radius:10px;">${d.corner}</div>`:'';
      const mk=function(s){return String(s).replace(/《([^》]*)》/g,'<span style="display:flex;color:#ff3a8a;">$1</span>');};
      return `<div style="width:1920px;height:1006px;box-sizing:border-box;position:relative;overflow:hidden;display:flex;flex-direction:column;justify-content:center;align-items:flex-start;padding-left:100px;background:linear-gradient(150deg,#ffd0e0,#ffe8c0);">`
        +right+corner
        +`<div style="position:relative;display:flex;align-items:center;margin-bottom:8px;"><div style="display:flex;background:#ff0000;color:#fff;font-size:54px;font-weight:900;padding:6px 26px;border-radius:14px;">${d.badge}</div></div>`
  ...
  { id:'note_s57', name:'note 単色＋キャラ＋付箋＋特大袋文字＋下帯', cat:'サムネ', fmt:'note',
    fields:[
      {key:'img',label:'左 キャラ(任意)',type:'file',def:''},
      {key:'badge',label:'付箋ラベル',def:'note版'},
      {key:'big',label:'特大（改行可）',def:'ライティングX\nマスター講座'},
      {key:'band',label:'下帯（《》黄マーカー）',def:'集客から《マネタイズ》まで！ちゃんと結果を出すためのX運用'}],
    render:(d,t)=>{const ic=d.img?`<div style="position:absolute;left:70px;bottom:120px;width:300px;height:300px;background:url(${d.img}) center/contain no-repeat;display:flex;"></div>`:'';
      const mk=function(s){return String(s).replace(/《([^》]*)》/g,'<span style="display:flex;background:#ffe24a;color:#1a1a1a;padding:0 6px;">$1</span>');};
  ...
  { id:'note_s60', name:'note イベントポスター型', cat:'サムネ', fmt:'note',
    fields:[
      {key:'img',label:'背景 写真(任意)',type:'file',def:''},
      {key:'brand',label:'左上 ブランド',def:'ABEMA'},
      {key:'big',label:'特大タイトル',def:'エイオキクラッチ01'},
      {key:'en',label:'英字サブ',def:'PROWRESLING MATCH | SHINJUKU FACE'},
      {key:'date',label:'日付',def:'4.20 2026'}],
    render:(d,t)=>{const bg=d.img?`background:url(${d.img}) center/cover;`:'background:linear-gradient(120deg,#0a2a5a,#04122e);';
      return `<div style="width:1920px;height:1006px;box-sizing:border-box;position:relative;overflow:hidden;display:flex;flex-direction:column;justify-content:flex-start;align-items:flex-start;padding:90px 80px 80px 80px;${bg}">`
        +`<div style="display:flex;font-size:50px;font-weight:900;letter-spacing:4px;color:#fff;margin-bottom:14px;">${d.brand}</div>`
        +`<div style="display:flex;font-size:150px;font-weight:900;color:#fff;line-height:1.02;${xstroke(9,'#0a1a3a')};">${d.big}</div>`
        +`<div style="display:flex;font-size:40px;font-weight:700;letter-spacing:4px;color:#cfe0ff;margin-top:18px;">${d.en}</div>`
        +`<div style="display:flex;align-items:center;margin-top:auto;"><div style="display:flex;width:14px;height:80px;background:#ffd24a;margin-right:24px;"></div><div style="display:flex;font-size:88px;font-weight:900;color:#fff;${xstroke(9,'#0a1a3a')};">${d.date}</div></div>`
      +`</div>`;} },

  { id:'note_s65', name:'note 図解教育型（中央2行＋下サブ）', cat:'サムネ', fmt:'note',
    fields:[
      {key:'img',label:'背景 イラスト(任意)',type:'file',def:''},
      {key:'big',label:'中央2行（《》強調・改行可）',def:'人間関係のモヤモヤを消す\n「《中和の法則》」'},
      {key:'sub',label:'下 サブ',def:'〜ネガティブな人への3つの対処法'}],
    render:(d,t)=>{const bg=d.img?`background:url(${d.img}) center/cover;`:'background:linear-gradient(160deg,#eef4fa,#dce8f2);';
      const mk=function(s){return String(s).replace(/《([^》]*)》/g,'<span style="display:flex;color:#1763c4;">$1</span>');};
      const lines=String(d.big).split('\n').map(function(l){return `<div style="display:flex;justify-content:center;">${mk(l)}</div>`;}).join('');
      return `<div style="width:1920px;height:1006px;box-sizing:border-box;position:relative;overflow:hidden;display:flex;flex-direction:column;justify-content:center;align-items:center;${bg}">`
        +`<div style="display:flex;flex-direction:column;align-items:center;font-size:96px;font-weight:900;color:#1a2a3a;line-height:1.2;text-shadow:0 2px 8px rgba(255,255,255,0.6);">${lines}</div>`
        +`<div style="display:flex;font-size:50px;font-weight:900;color:#3a4a5a;margin-top:18px;">${d.sub}</div>`
      +`</div>`;} },

  { id:'note_s66', name:'note 大見出し＋右下コーナーテロップ（写真任意）', cat:'サムネ', fmt:'note',
    fields:[
      {key:'img',label:'背景 写真(任意)',type:'file',def:''},
      {key:'eyebrow',label:'左上 ラベル(任意・空欄で非表示)',def:'映画レビュー'},
      {key:'big',label:'大見出し（《》金・改行可）',def:'観て終わりに\nしない《読み解き方》'},
      {key:'corner',label:'右下 テロップ',def:'観た後に自分を読む'}],
    render:(d,t)=>{const bg=d.img?`background:url(${d.img}) center/cover;`:'background:linear-gradient(120deg,#caa050,#5a3a20);';
      const mk=function(s){return String(s).replace(/《([^》]*)》/g,'<span style="display:flex;color:#ffd24a;">$1</span>');};
      const lines=String(d.big).split('\n').map(function(l){return `<div style="display:flex;align-items:center;">${mk(l)}</div>`;}).join('');
      const eb=String(d.eyebrow||'').trim()?`<div style="display:flex;align-self:flex-start;background:rgba(30,90,180,0.92);color:#fff;font-size:42px;font-weight:900;padding:8px 28px;margin-bottom:26px;">${d.eyebrow}</div>`:'';
      return `<div style="width:1920px;height:1006px;box-sizing:border-box;position:relative;overflow:hidden;display:flex;flex-direction:column;justify-content:center;align-items:flex-start;padding:0 120px 0 90px;${bg}font-family:${t.head};">`
        +`<div style="position:absolute;left:0;top:0;width:100%;height:100%;display:flex;background:linear-gradient(90deg,rgba(0,0,0,0.6),rgba(0,0,0,0.2) 55%,rgba(0,0,0,0));"></div>`
  ...
  { id:'note_s70', name:'note 学習LP（上帯＋2行＋下帯＋右学生）', cat:'サムネ', fmt:'note',
    fields:[
      {key:'img',label:'右 学生/PC(任意)',type:'file',def:''},
      {key:'top',label:'上 紫帯',def:'【大学受験を控えてる方は必見！】'},
      {key:'big',label:'2行（《》赤・改行可）',def:'Geminiの\n《有用プロンプト》を紹介'},
      {key:'band',label:'下 青帯',def:'勉強×AIで効率アップ'}],
    render:(d,t)=>{const right=d.img?`<div style="position:absolute;right:0;top:0;width:560px;height:1006px;background:url(${d.img}) center/cover;display:flex;"></div>`:'';
  ...
  { id:'yt_info_hero', name:'インフォ ヒーロー数字（線画＋黄マーカー）', cat:'サムネ', fmt:'youtube',
    fields:[{key:'icon',label:'アイコン名(lucide)',def:'plane'},{key:'label',label:'上 ラベル',def:'年間休日'},{key:'num',label:'大数字',def:'123日'},{key:'unit',label:'単位(任意)',def:'相当'},{key:'desc',label:'説明',def:'しっかり休めて、生活が充実する。'}],
    render:(d,t)=>infoHero(1280,720,d) },
  { id:'note_info_hero', name:'インフォ ヒーロー数字（線画＋黄マーカー）', cat:'サムネ', fmt:'note',
    fields:[{key:'icon',label:'アイコン名(lucide)',def:'plane'},{key:'label',label:'上 ラベル',def:'年間休日'},{key:'num',label:'大数字',def:'123日'},{key:'unit',label:'単位(任意)',def:'相当'},{key:'desc',label:'説明',def:'しっかり休めて、生活が充実する。'}],
    render:(d,t)=>infoHero(1920,1006,d) },

  { id:'yt_info_stat3', name:'インフォ 数字スタッツ 横3枚', cat:'サムネ', fmt:'youtube',
    fields:[{key:'title',label:'上 タイトル',def:'数字で見るはたらき方'},{key:'c1',label:'カード1（icon|ラベル|数字|説明）',def:'plane|年間休日|123日|しっかり休める'},{key:'c2',label:'カード2',def:'sprout|定着率|90.6%|長く働ける'},{key:'c3',label:'カード3',def:'baby|育休復職|98.7%|安心して戻れる'}],
    render:(d,t)=>infoStat3(1280,720,d) },
  { id:'note_info_stat3', name:'インフォ 数字スタッツ 横3枚', cat:'サムネ', fmt:'note',
    fields:[{key:'title',label:'上 タイトル',def:'数字で見るはたらき方'},{key:'c1',label:'カード1（icon|ラベル|数字|説明）',def:'plane|年間休日|123日|しっかり休める'},{key:'c2',label:'カード2',def:'sprout|定着率|90.6%|長く働ける'},{key:'c3',label:'カード3',def:'baby|育休復職|98.7%|安心して戻れる'}],
...
const infoCard=${infoCard.toString()};
const mark=${mark.toString()};
const infoHero=${infoHero.toString()};
const infoStat3=${infoStat3.toString()};
const infoIndex=${infoIndex.toString()};
const infoBannerNo1=${infoBannerNo1.toString()};
