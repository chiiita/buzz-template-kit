...
function infoHero(w,h,d){const cy='#1aa3d8';
  return `<div style="${infoCard(w,h,'#eaf6fb')}align-items:center;padding:0 ${Math.round(w*0.07)}px;">`
    ...
    +`<div style="display:flex;flex-direction:column;margin-left:${Math.round(w*0.05)}px;">`
      ...
      +`<div style="display:flex;font-size:${Math.round(h*0.045)}px;color:#5a7686;line-height:1.5;margin-top:${Math.round(h*0.03)}px;max-width:${Math.round(w*0.6)}px;">${d.desc||''}</div>`
    +`</div>`
  +`</div>`;}
...
function infoIndex(w,h,d){const items=String(d.items||'').split('\n').filter(function(x){return x.length;});
  ...
  return `<div style="${infoCard(w,h,'#ffffff')}flex-direction:column;padding:${Math.round(h*0.08)}px ${Math.round(w*0.08)}px;">`
...
function infoBannerNo1(w,h,d){const teal='#15b3c9';const right=d.img?`<div style="position:absolute;right:${Math.round(w*0.04)}px;top:50%;transform:translateY(-50%);width:${Math.round(w*0.34)}px;height:${Math.round(h*0.6)}px;background:url(${d.img}) center/contain no-repeat;display:flex;"></div>`:'';
  return `<div style="${infoCard(w,h,teal)}flex-direction:column;justify-content:center;padding:0 ${Math.round(w*0.06)}px;color:#fff;">`
    ...
    +`<div style="display:flex;align-items:center;margin-top:${Math.round(h*0.01)}px;">`
      +`<div style="display:flex;transform:scaleX(-1);">${icon('sprout','#eaffff',Math.round(h*0.2),1.6)}</div>`
      +`<div style="display:flex;font-size:${Math.round(h*0.32)}px;font-weight:900;line-height:1;margin:0 ${Math.round(w*0.01)}px;color:#fff;">${d.num||'No.1'}</div>`
      +`<div style="display:flex;">${icon('sprout','#eaffff',Math.round(h*0.2),1.6)}</div>`
    +`</div>`
...
const TEMPLATES = [
  ...
  { id:'note_s13', name:'note 暖色イラスト＋上 赤フチ2行', cat:'サムネ', fmt:'note',
    fields:[
      {key:'img',label:'背景 イラスト(任意)',type:'file',def:''},
      {key:'lines',label:'上 2行（《》赤・改行可）',def:'「《親バカ》」は最高の\n子育て戦略である'}],
    render:(d,t)=>{const bg=d.img?`background:url(${d.img}) center/cover;`:'background:linear-gradient(160deg,#fbe6cf,#f6d2b0);';
  ...
  { id:'note_s15', name:'note 黒金神秘＋上小＋金2行＋下赤帯', cat:'サムネ', fmt:'note',
    ...
    render:(d,t)=>{const bg=d.img?`background:url(${d.img}) center/cover;`:'background:radial-gradient(circle at 50% 42%,#4a3a10,#080604 72%);';
      return `<div style="width:1920px;height:1006px;box-sizing:border-box;position:relative;overflow:hidden;display:flex;flex-direction:column;justify-content:center;align-items:center;${bg}">`
        +`<div style="display:flex;font-size:48px;font-weight:700;color:#f0e0b0;margin-bottom:14px;">${d.top}</div>`
        +`<div style="display:flex;flex-direction:column;align-items:center;font-size:126px;font-weight:900;color:#e8c860;line-height:1.18;text-shadow:0 0 24px rgba(220,180,80,0.6);">${nl(d.big)}</div>`
        +`<div style="position:absolute;left:0;bottom:60px;width:100%;display:flex;justify-content:center;"><div style="display:flex;background:#b81818;color:#fff;font-size:40px;font-weight:900;padding:10px 34px;">${d.band}</div></div>`
      +`</div>`;} },

  { id:'note_s16', name:'note 水色＋左イラスト＋右白パネル＋丸数字', cat:'サムネ', fmt:'note',
    fields:[
    ...
    render:(d,t)=>{const ill=d.img?`<div style="position:absolute;left:70px;top:50%;transform:translateY(-50%);width:520px;height:680px;background:url(${d.img}) center/contain no-repeat;display:flex;"></div>`:'';
      return `<div style="width:1920px;height:1006px;box-sizing:border-box;position:relative;overflow:hidden;display:flex;justify-content:flex-end;align-items:center;padding-right:90px;background:#28c0dc;">`
        +ill
  ...
  { id:'note_s30', name:'note 黒金＋左アイコン＋角括弧帯＋金2行', cat:'サムネ', fmt:'note',
    ...
    render:(d,t)=>{const ic=d.img?`<div style="position:absolute;left:120px;top:50%;transform:translateY(-50%);width:300px;height:300px;background:url(${d.img}) center/contain no-repeat;display:flex;"></div>`:'';
      const mk=function(s){return String(s).replace(/《([^》]*)》/g,'<span style="display:flex;color:#e8c860;">$1</span>');};
      const lines=String(d.big).split('\n').map(function(l){return `<div style="display:flex;justify-content:center;">${mk(l)}</div>`;}).join('');
      return `<div style="width:1920px;height:1006px;box-sizing:border-box;position:relative;overflow:hidden;display:flex;flex-direction:column;justify-content:center;align-items:center;background:radial-gradient(circle at 55% 45%,#2a2410,#080604 72%);font-family:${t.head};">`
        +ic
        +`<div style="display:flex;align-items:center;border-top:2px solid #c8a850;border-bottom:2px solid #c8a850;padding:8px 40px;color:#e8d8a0;font-size:46px;font-weight:900;margin-bottom:22px;">${d.badge}</div>`
        +`<div style="display:flex;flex-direction:column;align-items:center;font-size:134px;font-weight:900;color:#fff;line-height:1.12;text-shadow:0 3px 14px rgba(0,0,0,0.5);">${lines}</div>`
        +`<div style="display:flex;font-size:48px;font-weight:900;color:#e8d8a0;margin-top:10px;">${d.tail}</div>`
      +`</div>`;} },
  ...
  { id:'note_s33', name:'note 写真＋下 毛筆2部', cat:'サムネ', fmt:'note',
    fields:[
      {key:'img',label:'背景 写真(任意)',type:'file',def:''},
      {key:'big',label:'下 毛筆（《》赤）',def:'《有料記事、》だからなんだ?'}],
  ...
  { id:'note_s42', name:'note クリーム＋金枠＋イラスト＋左下特大', cat:'サムネ', fmt:'note',
    fields:[
      {key:'img',label:'背景 イラスト(任意)',type:'file',def:''},
  ...
  { id:'note_s43', name:'note 2×2マトリクス図解（スライド型）', cat:'サムネ', fmt:'note',
    fields:[
      {key:'title',label:'タイトル',def:'イノベーションの類型化'},
      {key:'cells',label:'4象限 ラベル|項目,項目 を / 区切り（左上→右上→左下→右下）',def:'B|ウォークマン,ウォシュレット,CVS/A|電話,検索エンジン,iPhone/|/C|電報,新幹線,TESLA'},
      {key:'xa',label:'横軸ラベル',def:'解決のアプローチ'},
      {key:'ya',label:'縦軸ラベル',def:'問題の種類'}],
    render:(d,t)=>{const cs=String(d.cells).split('/');const cell=function(x){const p=x.split('|');const items=(p[1]||'').split(',').filter(Boolean).map(function(it){return `<div style="display:flex;align-items:center;font-size:38px;color:#1a2a3a;margin-top:6px;"><div style="display:flex;width:18px;height:18px;border-radius:50%;background:#1a2a3a;margin-right:14px;"></div>${it}</div>`;}).join('');
        return `<div style="display:flex;flex-direction:column;width:600px;height:280px;border:3px solid #16324a;padding:18px 26px;"><div style="display:flex;justify-content:center;font-size:46px;font-weight:900;color:#16324a;">${p[0]||''}</div>${items}</div>`;};
      return `<div style="width:1920px;height:1006px;box-sizing:border-box;position:relative;overflow:hidden;display:flex;flex-direction:column;justify-content:center;align-items:center;background:#ffffff;font-family:${t.head};color:#16324a;">`
        +`<div style="position:absolute;top:60px;left:90px;display:flex;font-size:60px;font-weight:900;">${d.title}</div>`
        +`<div style="display:flex;flex-direction:column;border:3px solid #16324a;">`
          +`<div style="display:flex;">${cell(cs[0]||'')}${cell(cs[1]||'')}</div>`
          +`<div style="display:flex;">${cell(cs[2]||'')}${cell(cs[3]||'')}</div>`
        +`</div>`
        +`<div style="display:flex;font-size:42px;font-weight:700;margin-top:24px;">${d.xa}　→</div>`
        +`<div style="position:absolute;left:60px;top:50%;display:flex;font-size:42px;font-weight:700;">${d.ya}</div>`
      +`</div>`;} },

  { id:'note_s46', name:'note ゲームリザルト型（左情報＋中央順位＋右チーム）', cat:'サムネ', fmt:'note',
    fields:[
      {key:'badge',label:'中央 バッジ',def:'チャンピオン級'},
      {key:'rank',label:'順位',def:'3位'},
      {key:'rate',label:'最終レート',def:'2677.551'},
      {key:'season',label:'シーズン',def:'シーズンM-1'},
      {key:'rule',label:'ルール',def:'ダブルバトル'},
      {key:'period',label:'期間',def:'2026/04/08〜05/13'},
      {key:'team',label:'チーム6体（カンマ）',def:'ガブリアス,カイリュー,リザードン,ドドゲザン,エルフーン,ニンフィア'}],
    render:(d,t)=>{const box=function(lb,vl){return `<div style="display:flex;flex-direction:column;background:rgba(255,255,255,0.1);border-radius:14px;padding:16px 24px;margin-bottom:18px;"><div style="display:flex;font-size:30px;color:#c8c0f0;">${lb}</div><div style="display:flex;font-size:42px;font-weight:900;color:#fff;">${vl}</div></div>`;};
      const team=String(d.team).split(',').map(function(x){return `<div style="display:flex;align-items:center;background:rgba(255,255,255,0.1);border-radius:12px;padding:8px 18px;margin-bottom:10px;"><div style="display:flex;width:48px;height:48px;border-radius:50%;background:#6a5aaa;margin-right:16px;"></div><div style="display:flex;font-size:34px;font-weight:700;color:#fff;">${x}</div></div>`;}).join('');
      return `<div style="width:1920px;height:1006px;box-sizing:border-box;position:relative;overflow:hidden;display:flex;align-items:center;justify-content:space-between;padding:50px 60px;background:linear-gradient(160deg,#4a3a8a,#241a52);font-family:${t.head};">`
        ...
        +`<div style="display:flex;flex-direction:column;align-items:center;width:560px;">`
          ...
          +`<div style="display:flex;font-size:64px;font-weight:900;color:#fff;">${d.rate}</div>`
        +`</div>`
        +`<div style="display:flex;flex-direction:column;width:460px;"><div style="display:flex;justify-content:flex-end;font-size:40px;font-weight:900;color:#fff;margin-bottom:16px;">チーム1</div>${team}</div>`
      +`</div>`;} },

  { id:'note_s55', name:'note YouTube副業型（ロゴ＋数字袋文字＋帯）', cat:'サムネ', fmt:'note',
    fields:[
      {key:'img',label:'右 キャラ(任意)',type:'file',def:''},
      {key:'corner',label:'右上 小',def:'顔出しなし・声出しなし'},
  ...
  { id:'note_s66', name:'note 写真＋右下 コーナーテロップ', cat:'サムネ', fmt:'note',
    fields:[
      {key:'img',label:'背景 写真(任意)',type:'file',def:''},
  ...
  { id:'yt_info_hero', name:'インフォ ヒーロー数字（線画＋黄マーカー）', cat:'サムネ', fmt:'youtube',
    fields:[{key:'icon',label:'アイコン名(lucide)',def:'plane'},{key:'label',label:'上 ラベル',def:'年間休日'},{key:'num',label:'大数字',def:'123日'},{key:'unit',label:'単位(任意)',def:'相当'},{key:'desc',label:'説明',def:'しっかり休んで生活を充実。趣味や家族との時間が、良い仕事につながる。'}],
