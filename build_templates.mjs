 ...
 *{box-sizing:border-box} body{margin:0;font-family:'Zen Kaku Gothic New',sans-serif;background:#2a2a2e;color:#eee;display:flex;height:100vh;overflow:hidden}
 ...
 #movebtn{background:#2D6CDF!important;flex:1 0 auto!important;min-width:96px} #movebtn.on{background:#1b9e4b!important}
 ...
 #movehint{display:none;position:absolute;bottom:12px;left:50%;transform:translateX(-50%);background:#1b9e4b;color:#fff;font-size:12px;padding:7px 16px;border-radius:8px;z-index:6;box-shadow:0 4px 14px rgba(0,0,0,.4);white-space:nowrap}
...
<div id="mid"><div id="preview"><div id="stage"><div id="card"></div></div><div id="ovwarn">⚠ 文字がはみ出している可能性</div><div id="movehint">✋ 移動・編集モード中 — 文字や表をクリックで選択 → ドラッグで移動／Deleteキーで削除</div><div id="ver">build 2026-06-11b</div></div><div id="deckbar"></div></div>
<div id="form"><div id="bar"><button onclick="doPrint()">デッキをPDF保存</button><button class="sec" id="undobtn" onclick="undo()" title="元に戻す (Ctrl+Z)" disabled>↶</button><button class="sec" id="redobtn" onclick="redo()" title="やり直し (Ctrl+Y)" disabled>↷</button><button class="sec" id="movebtn" onclick="toggleMove()">✋移動</button><button class="sec" onclick="togglePresets()">構成</button><button class="sec" onclick="toggleIcons()">アイコン</button><button class="sec" onclick="newDeck()">新規</button><button class="sec" onclick="exportDeck()">書出</button><button class="sec" onclick="document.getElementById('imp').click()">読込</button><input id="imp" type="file" accept="application/json" style="display:none" onchange="importDeck(event)"></div><div id="slideopt"></div><div id="fields"></div>
...
function toggleMove(){moveMode=!moveMode;selBlock=-1;selFree=-1;document.getElementById('movebtn').classList.toggle('on',moveMode);document.getElementById('movebtn').textContent=moveMode?'✋移動 中':'✋移動';document.getElementById('movehint').style.display=moveMode?'block':'none';buildSlideOpt();renderCard();}
